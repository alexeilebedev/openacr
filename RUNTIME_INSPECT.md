...
amc knows every pool in FDb via the `zd_inst` linked list (discovery in `cpp/amc/gen.cpp:460`, checks `reftype.inst` flag). 152 pool-typed fields exist across all FDb structs. A new `amcdb.gen` phase could generate a state-dump function -- one record in `gen.ssim`, one generator, every program gets it. Passes the factorization test.

**Print gap (verified):** Print is NOT generated for every ctype. Only 799 of 1413 ctypes have `cfmt` entries with `print:Y`. 616 have no cfmt at all (mostly extern types like DIR, SSL_CTX). 18 explicitly opt out. The generator must serialize field-by-field from the schema directly, not depend on cfmt/Print.
...
**Phase 1 -- Pool census.** Generate a function that emits ctype name + record count for every pool in FDb. One line per pool. Zero serialization, zero Print dependency -- just counts that amc already tracks (`_nalloc`). Output: `ns.FCtype n:423`. Smallest useful increment. Single `amcdb.gen` record + one generator function.
...
91 generators exist in `data/amcdb/gen.ssim`. Adding a new one requires: 1 ssimfile record (`amcdb.gen gen:pool_census perns:Y`), 1 C++ function (`amc::gen_pool_census`). Uniform `void()` contract. The dispatch loop in `cpp/amc/main.cpp:1231` calls all generators via function pointers -- no special cases needed.
