## dev - Tables supporting compilation of programs


### Chapters
<a href="#chapters"></a>

<!-- TOC_BEG AUTO -->
* [dev.arch - System architecture](/txt/ssimdb/dev/arch.md)

* [dev.badline - Regex of a dubious source code line](/txt/ssimdb/dev/badline.md)

* [dev.builddir - Directory where object files/executables go. Determines compile/link options](/txt/ssimdb/dev/builddir.md)

* [dev.cfg - Compiler configuration](/txt/ssimdb/dev/cfg.md)

* [dev.compiler - One of the known compilers](/txt/ssimdb/dev/compiler.md)

* [dev.copyright - Copyrighting entity](/txt/ssimdb/dev/copyright.md)

* [dev.covfile -](/txt/ssimdb/dev/covfile.md)

* [dev.covline -](/txt/ssimdb/dev/covline.md)

* [dev.covtarget -](/txt/ssimdb/dev/covtarget.md)

* [dev.edaction - acr_ed editing action (only one can exist at a time)](/txt/ssimdb/dev/edaction.md)

* [dev.edacttype - acr_ed editing action type](/txt/ssimdb/dev/edacttype.md)

* [dev.gitfile - File managed by git](/txt/ssimdb/dev/gitfile.md)

* [dev.gitinfo -](/txt/ssimdb/dev/gitinfo.md)

* [dev.hilite - Mapping of perl regx->hilite pattern](/txt/ssimdb/dev/hilite.md)

* [dev.htmlentity - Known HTML entity strings & codes](/txt/ssimdb/dev/htmlentity.md)

* [dev.include - A site where one file includes another](/txt/ssimdb/dev/include.md)

* [dev.license - Source code license](/txt/ssimdb/dev/license.md)

* [dev.linelim -](/txt/ssimdb/dev/linelim.md)

* [dev.mdsection - Template for updating a markdown file](/txt/ssimdb/dev/mdsection.md)

* [dev.msgfile - File in repo containing messages that should be normalized during CI](/txt/ssimdb/dev/msgfile.md)

* [dev.noindent - Indicates that a file should not be automatically indented](/txt/ssimdb/dev/noindent.md)

* [dev.opt_type - Type of option (used in tool_opt)](/txt/ssimdb/dev/opt_type.md)

* [dev.package - OpenACR package](/txt/ssimdb/dev/package.md)

* [dev.pkgdep - OpenACR Package dependency](/txt/ssimdb/dev/pkgdep.md)

* [dev.pkgkey - Keys belonging to the OpenACR package](/txt/ssimdb/dev/pkgkey.md)

* [dev.readme - File containing documentation](/txt/ssimdb/dev/readme.md)

* [dev.readmecat - sorted categories of txt/ *.md files presented in dev.readme](/txt/ssimdb/dev/readmecat.md)

* [dev.sandbox - Registered sandbox](/txt/ssimdb/dev/sandbox.md)

* [dev.sbpath - Extra files to copy into the sandbox](/txt/ssimdb/dev/sbpath.md)

* [dev.scriptfile - Known script file](/txt/ssimdb/dev/scriptfile.md)

* [dev.srcfile - Source file or header (key is pathname)](/txt/ssimdb/dev/srcfile.md)

* [dev.syscmd - System command to execute](/txt/ssimdb/dev/syscmd.md)

* [dev.syscmddep - Dependency between two commands](/txt/ssimdb/dev/syscmddep.md)

* [dev.syslib - Registered system library](/txt/ssimdb/dev/syslib.md)

* [dev.targdep - Dependency between targets](/txt/ssimdb/dev/targdep.md)

* [dev.target - Build target](/txt/ssimdb/dev/target.md)

* [dev.targsrc - List of sources for target](/txt/ssimdb/dev/targsrc.md)

* [dev.targsyslib - Use of system library by target](/txt/ssimdb/dev/targsyslib.md)

* [dev.tgtcov - Captured line coverate information by target](/txt/ssimdb/dev/tgtcov.md)

* [dev.timefmt - Time formats supported by orgfile](/txt/ssimdb/dev/timefmt.md)

* [dev.tool_opt - Compiler/linker options to use](/txt/ssimdb/dev/tool_opt.md)

* [dev.uname - List of known unames](/txt/ssimdb/dev/uname.md)

* [dev.unstablefld - Fields that should be stripped from component test output because they contain timestamps etc.](/txt/ssimdb/dev/unstablefld.md)

<!-- TOC_END AUTO -->

### Table Of Contents
<a href="#table-of-contents"></a>
* [Chapters](#chapters)

