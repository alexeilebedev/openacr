...
### Tests
<a href="#tests"></a>
The following component tests are defined for `src_hdr`.
These can be executed with `atf_comp <comptest> -v`
|Comptest|Comment|
|---|---|
|[src_hdr.Converge](/test/atf_comp/src_hdr.Converge)|the second header rewrite of a headerless script is a no-op|
|[src_hdr.FileReadFail](/test/atf_comp/src_hdr.FileReadFail)|a missing source file fails the run naming the path|
|[src_hdr.FileWriteFail](/test/atf_comp/src_hdr.FileWriteFail)|a failed header write fails the run naming the path|
|[src_hdr.NoCmtstring](/test/atf_comp/src_hdr.NoCmtstring)|a scriptfile with no comment syntax fails the run byte-unchanged|
|[src_hdr.UpdateprotoFail](/test/atf_comp/src_hdr.UpdateprotoFail)|a failed src_func -updateproto step fails the run|
