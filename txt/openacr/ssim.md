...
Some common recipes are available in [Ssimfiles](/txt/openacr/recipes.md) and all ssimfiles are described in [Ssim Databases](/txt/ssimdb/README.md).
...
#### Every subset of a **ssimfile** is a **ssimfile**
<a href="#every-subset-of-a-ssimfile-is-a-ssimfile-"></a>
Every line repeats its own field names beside its values, and that repetition is what buys the format its closure.
Take any lines out of a **ssimfile** and what is left is a **ssimfile**, concatenate two of them and the result is one, and the empty file is one as well.
There is no header, no footer and no schema declaration, so there is nothing a fragment can be missing.

That closure is why the ordinary shell tools work on this data.
`head`, `grep`, a `sed` filter and `cat` of two files each produce valid input for anything that reads **ssim**.
No tool has to know whether it was handed a whole document or a piece of one, because the two are not distinguishable.

The repetition looks like waste, and the optimization that removes it is easy to see.
Hoist the field names into a schema line at the top of the file, and the format saves those bytes on every line after it.
What that costs is the closure: a subset of such a file is no longer a file, every reader has to know whether it holds a complete document, and every pipeline has to carry the header along.
The bytes are the price the format pays, deliberately, for the property.

The argument is made at length in the ssim paper, under *Finding the generator*.
The papers live in their own repository, `algornd/paper`, rather than in this tree.
...
Each **field** in a **ssimfile** tuple is a **key:value** pair. The **ssimfile** definitions, described in [How to define a new ssimfile](/txt/openacr/recipes.md) allow a **field** to be partitioned into additional **fldfunc** **fields** of **key:value** pair format. These additional **fields** provide **keys** (names) for the substrings of the **value** of the original **key:value** pair.

The **value** string is subdivided into substrings, with the explicit separator character, defined and visible in the string. The substrings provide new **values**, with the new **keys** coming from **ssimfile** definitions. The definitions creation is described here:[How to define a new ssimfile](/txt/openacr/recipes.md).
...
here the value of``dmmeta.ctype:dev.Cfg`` is expanded into two additional **fldfunc fields** ``ns:dev and name:Cfg``
