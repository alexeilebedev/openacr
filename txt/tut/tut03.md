...
> and fields, so running them in your own checkout changes its
> schema and its sources.  Reproduce the session in a sandbox
> instead: `wt tut_make -reset "bash -l"` gives you a shell in
> one, reset to match your current directory, and nothing you do
> there reaches your working tree — see
> [wt](/txt/exe/wt/README.md).

`amc` supports two flavors of application.

- **RPC-style clients** are written in the *logical-time*
  domain: one source line ≈ one logical step of the algorithm,
  blocking calls are the norm.
- **Engines** are real-time, cooperatively-scheduled
  applications written in the *real-time* domain: one source
  line ≈ a handful of CPU instructions, no blocking calls.  In
  full kernel-bypass mode, even system calls are forbidden;
  amc and `algo_lib` support this.

Mixing is allowed but the distinction is useful.  Keeping
engines and RPC code separate gives you the best of both —
engine throughput and RPC clarity.

For the full conceptual model (`next_loop`, smooth blocking/
hot-polling, every step type and what it generates), see
[Runtime](/txt/exe/amc/runtime.md).  This tutorial walks
through building an engine-style program from scratch.
...
acr.insert  dmmeta.ctype  ctype:command.samp_tut3  comment:""
  acr.insert  dmmeta.field  field:command.samp_tut3.in  arg:algo.cstring  reftype:Val  dflt:'"data"'  comment:"Input directory or filename, - for stdin"
    acr.insert  dmmeta.floadtuples  field:command.samp_tut3.in  autoscan:Y  optional:N  comment:""

  acr.insert  dmmeta.ccmdline  ctype:command.samp_tut3  read:Y  basecmdline:algo_lib.FDb.cmdline  comment:""
...
### Going further: I/O and framed messages
<a href="#going-further-i-o-and-framed-messages"></a>

Once steps are firing on schedule, the next question is how
external events — a file descriptor becoming readable, a peer
sending a message — fold into the same loop without
per-iteration polling code.

- For the low-level fd-event API
  (`IohookAdd` / `IohookRemove`, edge-triggered semantics,
  cross-platform behavior), see
  [Iohook reference](/txt/lib/algo_lib/FIohook.md).
- For framed protocol I/O on top of iohook (length-delimited
  messages landing in a typed inbox), see
  [Tutorial 6: Fbuf](/txt/tut/tut06.md) and the
  [amc Fbuf feature page](/txt/exe/amc/fbuf.md).
