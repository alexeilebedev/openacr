## Tutorial 5: Running N parallel subprocesses
<a href="#tutorial-5-running-n-parallel-subprocesses"></a>

> **Live tutorial.**  The code listings and command outputs
> below are pulled from the live `atf_nrun` source by
> `abt_md`'s `inline-command:` evaluator — they show the
> program as it actually exists in the tree, not a copy that
> can drift.  Reproduce them by running the same commands
> against your checkout.

A worked example to accompany
[Tutorial 3: Scheduling and main loop](/txt/tut/tut03.md).
We describe `atf_nrun`, a small program that runs N child
jobs in parallel — spawning new jobs as old ones complete,
polling `waitpid` only when SIGCHLD says it's worth it.
Everything is driven from steps, in the real-time domain.
