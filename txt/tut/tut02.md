...
This tutorial introduces the idea of storing data in plain
text files by analogy with SQL database tables, then extends
it to the *schema* — the description of the data — using
those same files.
...
system won't do, because we already have a lot of data in `ssimfiles`, and we have the `ctypes` to handle
...
additional concepts, such as a physical set of C++ files on disk, or a set of dev and production environments
...
and powerful development environment.
    ...
    'amc_gc acr -key:ctype:acr.%' analyzes and eliminates unused records in acr
    'amc_gc acr -key:ctype:amc_gc.%' analyzes and eliminates unused records in amc_gc
