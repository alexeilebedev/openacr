## Tutorial 8: ACR — interactive walkthrough
<a href="#tutorial-8-acr-interactive-walkthrough"></a>

> **Live tutorial, and it edits the tree.**  Every command shown
> below is executed in a sandbox by `abt_md` when this page is
> rebuilt, and the output you see is the actual output of that
> run.  The commands insert, replace and delete records and edit
> the schema, so running them in your own checkout changes it.
> Reproduce the session in a sandbox instead: `wt tut_make
> -reset "bash -l"` gives you a shell in one, reset to match
> your current directory, and nothing you do there reaches your
> working tree — see [wt](/txt/exe/wt/README.md).

This tutorial introduces `acr` interactively — query, insert,
replace, delete, schema editing — using a couple of throwaway
ssimfiles in the `dev` namespace.
...
report.acr_check  records:***  n_err:3
dev.b  a:xyz  comment:""
report.acr  n_select:***  n_insert:***  n_delete:***  n_ignore:***  n_update:***  n_file_mod:***  n_badline:0
```
