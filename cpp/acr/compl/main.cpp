...
bool acr_compl::CmdArgValueRequiredQ(acr_compl::FField &field) {
    return field.c_falias
        ? CmdArgValueRequiredQ(*field.c_falias->p_basefield)
...
void acr_compl::MarkSeen(acr_compl::FField &field) {
    field.seen = true;
    if (field.c_falias) {
        field.c_falias->p_basefield->seen = true;
    }
    if (field.c_falias_base) {
        field.c_falias_base->p_field->seen = true;
    }
}
...
bool acr_compl::CumulativeQ(acr_compl::FField &field) {
    FField *basefield = field.c_falias ? field.c_falias->p_basefield : NULL;
    return (field.c_fflag && field.c_fflag->cumulative)
        || (basefield && CumulativeQ(*basefield));
}
...
void acr_compl::Main() {
    ...
    dbglog(_db.cmdline);
    // load schema
    vrfy(LoadTuplesMaybe(_db.cmdline.schema), algo_lib::_db.errtext);
    if (!_db.is_data_dir) {
