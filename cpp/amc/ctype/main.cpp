...
void amc::tfunc_Ctype_Eq() {
    ...
    if (ctype.c_ccmp) {
        ...
        if (ctype.c_cpptype && ctype.c_cpptype->cheap_copy) {
            Ins(&R, opeq.proto, "$Name_Eq($ByvalArgtype lhs, $ByvalArgtype rhs)", false);
        } else {
            Ins(&R, opeq.proto, "$Name_Eq(const $ByvalArgtype lhs,const $ByvalArgtype rhs)", false);
        }
        opeq.extrn = ctype.c_ccmp->extrn;
        if (!ctype.c_ccmp->extrn) {
            opeq.inl = c_datafld_N(ctype) < 4;
            Ins(&R, opeq.body, "bool retval = true;");
            bool need_test = false;
            ind_beg(amc::ctype_c_datafld_curs, field,ctype) {
                tempstr test;
                if (PadQ(field)) {
                    ...
                    // This fails to use smallstr's specialize comparison function -- except
                    // it doesn't because smallstr defines Eq
                    Set(R, "$name", name_Get(field));
...
void amc::tfunc_Ctype_EqStrptr() {
    ...
    if (ctype.c_ccmp && single_smallstr) {
        ...
        amc::FFunc& opeq = amc::CreateCurFunc();
        Ins(&R, opeq.ret  , "bool", false);
        Ins(&R, opeq.proto, "$Name_EqStrptr($ByvalArgtype lhs, const algo::strptr &rhs)", false);
        opeq.inl = true;
        ind_beg(amc::ctype_c_datafld_curs, field,ctype) {
            if (field.reftype == dmmeta_Reftype_reftype_Smallstr) {// default for small strings
                Set(R, "$name", name_Get(field));
                Ins(&R, opeq.body, "return algo::strptr_Eq($name_Getary(lhs), rhs);");
            } else {
                vrfy(0,"error");
            }
        }ind_end;
...
void amc::tfunc_Ctype_ToCmdline() {
    ...
    if (has_print) {
        ...
        Ins(&R, tocmdline.ret    , "tempstr",false);
        Ins(&R, tocmdline.proto  , "$Name_ToCmdline($ByvalArgtype row)",false);
        Ins(&R, tocmdline.comment, "Convenience function that returns a full command line");
        Ins(&R, tocmdline.comment, "Assume command is in a directory called bin");
        Ins(&R, tocmdline.body   , "tempstr ret;");
        Ins(&R, tocmdline.body   , "ret << \"bin/$Name \";");
        Ins(&R, tocmdline.body   , "$Name_PrintArgv(row, ret);");
        Ins(&R, tocmdline.body   , "// inherit less intense verbose, debug options");
        Ins(&R, tocmdline.body   , "for (int i = 1; i < algo_lib::_db.cmdline.verbose; i++) {");
        Ins(&R, tocmdline.body   , "    ret << \" -verbose\";");
        Ins(&R, tocmdline.body   , "}");
        Ins(&R, tocmdline.body   , "for (int i = 1; i < algo_lib::_db.cmdline.debug; i++) {");
