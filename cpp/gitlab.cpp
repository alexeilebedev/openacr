...
static void Main_ManageEnv() {
    ...
    if (Pathcomp(gitlab::_db.cmdline.target.expr,".LL")==gitlab::_db.cmdline.target.expr){
        tempstr new_target;
        new_target<<gitlab::_db.cmdline.target.expr<<".%";
        Regx_ReadSql(gitlab::_db.cmdline.target, new_target, true);
    }
}
...
static void LoadProjects() {
    ...
    do {
        ...
        gitlab::FHttp http;
        http.request_uri << gitlab::_db.cmdline.host << "/api/v4"
                         << "/projects?per_page=100&page="<<page;
        // execute request
        CurlExec(http, http_auth_header);

        // prechecks on output
        vrfy(http.response_status_code == 200, tempstr() << "Host request has failed:"
        ...
        ind_beg(lib_json::node_c_child_curs,proj_obj,*http.response_json_parser.root_node){
            ...
            tempstr access(strptr_Get(&proj_obj,"permissions.group_access.access_level"));
            if (access!=""){
                ...
                gitlab_auth.url=gitlab::_db.cmdline.host;
            }
        }ind_end;
...
static void Main_ManageAuth() {
    ...
    if (gitlab::_db.cmdline.auth_token.ch_n) {
        // need host if the token is specified
        vrfy(gitlab::_db.cmdline.host.ch_n,tempstr()
             <<Keyval("host",gitlab::_db.cmdline.host)
             <<Keyval("comment","gitlab host have to be supplied with auth_token for init")
             );
        LoadProjects();
        exit(0);
    } else {
        // Find the first active auth token matching the project
        ind_beg(gitlab::_db_gitlab_auth_curs,gitlab_auth,gitlab::_db) if (gitlab_auth.active){
            tempstr proj_dot;
            proj_dot<<proj_Get(gitlab_auth)<<".";
            if (algo::StartsWithQ(gitlab::_db.cmdline.target.expr,proj_dot)
    ...
    vrfy(gitlab::_db.p_project,tempstr()
         <<Keyval("target",gitlab::_db.cmdline.target.expr)
         <<Keyval("mraccept",gitlab::_db.cmdline.mraccept)
         <<Keyval("auth_file",gitlab::_db.auth_file)
         <<eol
         <<Keyval("comment","no project matching auth_file records is selected by target or mraccept parameters")
         <<eol
         <<Keyval("comment","to initialize auth_file record please supply personal access token via -auth_token and -host option")
         <<eol
...
static void Main_ShowIssueList() {
    ...
    ind_beg(gitlab::_db_issue_curs, issue, gitlab::_db) {
        issue.select = algo_lib::Regx_Match(gitlab::_db.cmdline.target, issue.issue)
            || EndsWithQ(issue.issue, gitlab::_db.cmdline.target.expr);
        ...
        if (gitlab::_db.cmdline.assignee.expr == "" && gitlab::_db.cmdline.target.expr == "") {
...
static void Main_DoIssueAdd(gitlab::FProject &project) {
    ...
    vrfy(http.response_status_code == 201, tempstr() << "Host request has failed:"
         << http.response_status_line);
    ...
    gitlab::_db.cmdline.target.expr = issue.issue;
}
...
int gitlab::IssueArgNumber() {
    strptr expr = Pathcomp(_db.cmdline.target.expr, ".RR");
    i32 ret=0;
...
static void Main_DoIssueStart(gitlab::FProject &project) {
    ...
    vrfy(gitlab::_db.cmdline.target.expr != "", "Please specify issue ID");
    int issuenum = gitlab::IssueArgNumber();
