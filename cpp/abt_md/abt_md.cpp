...
void abt_md::LoadReadme(abt_md::FReadme &readme) {
    ...
    ind_beg(algo::FileLine_curs,line,readme.gitfile) {
        if (StartsWithQ(line,"##") || !cur_section) {
            ...
            verblog(readme.gitfile<<":"<<cur_section->firstline<<": begin section "<<line);
...
void abt_md::UpdateSection(abt_md::FileSection &file_section) {
    ...
    ind_beg(algo::Line_curs,line,file_section.text) {
        ...
        if (line=="" && leadempty) {
        ...
        } else if (!backticks && StartsWithQ(line,"inline-command: ")) {
            out << line << eol;
            ...
            // backticks: switch in and out of the backtick mode
            // backticks terminate inline command output
            out << line << eol;
            inlinecommand=false;
            backticks = !backticks;
        } else {
    ...
    if (backticks) {
        prerr(readme.gitfile<<":"<<file_section.firstline<<": unterminated backticks");
        algo_lib::_db.exit_code=1;
    }
...
void abt_md::mdsection_Tests(abt_md::FileSection &section) {
    section.text = "";// replace section text
    tempstr out = SysEval(Subst(_db.R,"acr comptest:$ns.% | ssimfilt -t -field comptest -field comment"),FailokQ(true),1024*1024);
    if (Trimmed(out)!="") {
...
void abt_md::Main() {
    ...
    if (!_db.cmdline.dry_run && !_db.cmdline.print && algo_lib::_db.exit_code==0) {
