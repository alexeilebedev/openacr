...
void atf_ci::CheckCleanDirs(strptr dirs) {
...
static void prep_env() {
    // custom environment for tests needing it
}

static bool RunCheck(atf_ci::FCitest &citest) {
    prlog("atf_ci.begin"
          <<Keyval("citest",citest.citest));
    bool success=true;
    algo::UnTime start=algo::CurrUnTime();
    try {
        if (!atf_ci::_db.cmdline.skipenv && citest.needenv && bool_Update(atf_ci::_db.preped_env, true)) {
            prep_env();
        }
...
void atf_ci::Main() {
    ...
    if (n_run==0) {
        ...
        prlog("atf_ci.nomatch"
              ...
              <<Keyval("comment","no tests matched specified pattern. run 'acr citest' to see the full list"));
    }
