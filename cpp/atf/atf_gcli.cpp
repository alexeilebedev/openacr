...
// 2. A "fine grained" "access token" key for the project is generated: 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 - also shows public projects r/o!
...
static void PrepGitdir(){
    ind_beg(atf_gcli::_db_gclienv_curs,gclienv,atf_gcli::_db) if (gclienv.gclienv!="all"){
        ...
        if (repo!=""){
            ...
            Ins(&R,cmd,"cp .git/config  temp/.$gitdir/.git/config");
            ...
            SysCmd(cmd,FailokQ(false),DryrunQ(atf_gcli::_db.cmdline.dry_run),EchoQ(algo_lib::_db.cmdline.verbose));
        }
    }ind_end;
}
...
void atf_gcli::Main() {
    ...
    ind_beg(atf_gcli::_db_c_gtblacttst_curs,gtblacttst,atf_gcli::_db) {
        ...
        // Issue command
        SysCmd(gtblacttst.cmd,FailokQ(true),DryrunQ(atf_gcli::_db.cmdline.dry_run),EchoQ(algo_lib::_db.cmdline.verbose));
        // Capture artefacts
