...
// --- gitlab.FDb._db.SaveTuples
// Save ssim data to given directory.
u32 gitlab::SaveTuples(algo::strptr root) {
    u32 retval = 0;
    u32 nbefore = algo_lib::_db.stringtofile_nwrite;
    (void)gitlab_auth_SaveSsimfile(SsimFname(root, "dev.gitlab_auth"));
...
// --- gitlab.FDb.gitlab_auth.SaveSsimfile
// Save table to ssimfile
bool gitlab::gitlab_auth_SaveSsimfile(algo::strptr fname) {
    cstring text;
    ind_beg(gitlab::_db_gitlab_auth_curs, gitlab_auth, gitlab::_db) {
        dev::GitlabAuth out;
        gitlab_auth_CopyOut(gitlab_auth, out);
        dev::GitlabAuth_Print(out, text);
        text << eol;
    }ind_end;
