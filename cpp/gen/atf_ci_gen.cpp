//
// cpp/gen/atf_ci_gen.cpp
// Generated by AMC
//
// (C) AlgoEngineering LLC 2008-2013
// (C) NYSE | Intercontinental Exchange 2013-2016
//


#include "include/algo.h"  // hard-coded include
#include "include/gen/atf_ci_gen.h"
#include "include/gen/atf_ci_gen.inl.h"
#include "include/gen/dev_gen.h"
#include "include/gen/dev_gen.inl.h"
#include "include/gen/algo_gen.h"
#include "include/gen/algo_gen.inl.h"
#include "include/gen/atfdb_gen.h"
#include "include/gen/atfdb_gen.inl.h"
#include "include/gen/command_gen.h"
#include "include/gen/command_gen.inl.h"
#include "include/gen/dmmeta_gen.h"
#include "include/gen/dmmeta_gen.inl.h"
#include "include/gen/lib_json_gen.h"
#include "include/gen/lib_json_gen.inl.h"
#include "include/gen/lib_prot_gen.h"
#include "include/gen/lib_prot_gen.inl.h"
#include "include/gen/algo_lib_gen.h"
#include "include/gen/algo_lib_gen.inl.h"
#include "include/gen/lib_git_gen.h"
#include "include/gen/lib_git_gen.inl.h"
//#pragma endinclude

// Instantiate all libraries linked into this executable,
// in dependency order
lib_json::FDb   lib_json::_db;    // dependency found via dev.targdep
algo_lib::FDb   algo_lib::_db;    // dependency found via dev.targdep
lib_git::FDb    lib_git::_db;     // dependency found via dev.targdep
atf_ci::FDb     atf_ci::_db;      // dependency found via dev.targdep

namespace atf_ci {
const char *atf_ci_help =
"atf_ci: Normalization tests (see citest table)\n"
"Usage: atf_ci [options]\n"
"    -in       string  Input directory or filename, - for stdin. default: \"data\"\n"
"    [citest]  string  Regx of tests to run. default: \"%\"\n"
"    -maxerr   int     Exit after this many errors. default: 0\n"
"    -cijob    string  . default: \"%\"\n"
"    -capture          Capture the output of the test. default: false\n"
"    -skipenv          Do not run prepenv for any test. default: false\n"
"    -verbose          Enable verbose mode\n"
"    -debug            Enable debug mode\n"
"    -version          Show version information\n"
"    -sig              Print SHA1 signatures for dispatches\n"
"    -help             Print this screen and exit\n"
;


const char *atf_ci_syntax =
"-in:string=\"data\"\n"
" [citest]:string=\"%\"\n"
" -maxerr:int=0\n"
" -cijob:string=\"%\"\n"
" -capture:flag\n"
" -skipenv:flag\n"
;
} // namespace atf_ci
namespace atf_ci { // gsymbol:atf_ci/atfdb.cijob
    const atfdb::CijobPkey atfdb_cijob_comp("comp");
    const atfdb::CijobPkey atfdb_cijob_memcheck("memcheck");
    const atfdb::CijobPkey atfdb_cijob_normalize("normalize");
}
namespace atf_ci { // gsymbol:atf_ci/dev.gitfile
}
namespace atf_ci { // gsymbol:atf_ci/dev.scriptfile
    const char* dev_scriptfile_bin_fix_gen_conflicts("bin/fix-gen-conflicts");
    const char* dev_scriptfile_bin_update_gitfile("bin/update-gitfile");
    const char* dev_scriptfile_bin_update_hdr("bin/update-hdr");
    const char* dev_scriptfile_bin_update_outfile("bin/update-outfile");
    const char* dev_scriptfile_bin_update_scriptfile("bin/update-scriptfile");
}
namespace atf_ci {
    // Load statically available data into tables, register tables and database.
    static void          InitReflection();
    static void          citest_LoadStatic() __attribute__((nothrow));
    static bool          ssimfile_InputMaybe(dmmeta::Ssimfile &elem) __attribute__((nothrow));
    static bool          scriptfile_InputMaybe(dev::Scriptfile &elem) __attribute__((nothrow));
    static bool          ns_InputMaybe(dmmeta::Ns &elem) __attribute__((nothrow));
    static bool          readme_InputMaybe(dev::Readme &elem) __attribute__((nothrow));
    static bool          builddir_InputMaybe(dev::Builddir &elem) __attribute__((nothrow));
    static bool          cfg_InputMaybe(dev::Cfg &elem) __attribute__((nothrow));
    static bool          gitfile_InputMaybe(dev::Gitfile &elem) __attribute__((nothrow));
    static bool          noindent_InputMaybe(dev::Noindent &elem) __attribute__((nothrow));
    static bool          targsrc_InputMaybe(dev::Targsrc &elem) __attribute__((nothrow));
    static bool          msgfile_InputMaybe(dev::Msgfile &elem) __attribute__((nothrow));
    // find trace by row id (used to implement reflection)
    static algo::ImrowPtr trace_RowidFind(int t) __attribute__((nothrow));
    // Function return 1
    static i32           trace_N() __attribute__((__warn_unused_result__, nothrow, pure));
    static void          SizeCheck();
} // end namespace atf_ci

// --- atf_ci.FBuilddir.base.CopyOut
// Copy fields out of row
void atf_ci::builddir_CopyOut(atf_ci::FBuilddir &row, dev::Builddir &out) {
    out.builddir = row.builddir;
    out.comment = row.comment;
}

// --- atf_ci.FBuilddir.base.CopyIn
// Copy fields in to row
void atf_ci::builddir_CopyIn(atf_ci::FBuilddir &row, dev::Builddir &in) {
    row.builddir = in.builddir;
    row.comment = in.comment;
}

// --- atf_ci.FBuilddir.uname.Get
algo::Smallstr50 atf_ci::uname_Get(atf_ci::FBuilddir& builddir) {
    algo::Smallstr50 ret(algo::Pathcomp(builddir.builddir, ".LL-LL"));
    return ret;
}

// --- atf_ci.FBuilddir.compiler.Get
algo::Smallstr50 atf_ci::compiler_Get(atf_ci::FBuilddir& builddir) {
    algo::Smallstr50 ret(algo::Pathcomp(builddir.builddir, ".LL-LR"));
    return ret;
}

// --- atf_ci.FBuilddir.cfg.Get
algo::Smallstr50 atf_ci::cfg_Get(atf_ci::FBuilddir& builddir) {
    algo::Smallstr50 ret(algo::Pathcomp(builddir.builddir, ".LR-LL"));
    return ret;
}

// --- atf_ci.FBuilddir.arch.Get
algo::Smallstr50 atf_ci::arch_Get(atf_ci::FBuilddir& builddir) {
    algo::Smallstr50 ret(algo::Pathcomp(builddir.builddir, ".LR-LR"));
    return ret;
}

// --- atf_ci.FBuilddir..Uninit
void atf_ci::FBuilddir_Uninit(atf_ci::FBuilddir& builddir) {
    atf_ci::FBuilddir &row = builddir; (void)row;
    ind_builddir_Remove(row); // remove builddir from index ind_builddir
}

// --- atf_ci.FCfg.base.CopyOut
// Copy fields out of row
void atf_ci::cfg_CopyOut(atf_ci::FCfg &row, dev::Cfg &out) {
    out.cfg = row.cfg;
    out.suffix = row.suffix;
    out.comment = row.comment;
}

// --- atf_ci.FCfg.base.CopyIn
// Copy fields in to row
void atf_ci::cfg_CopyIn(atf_ci::FCfg &row, dev::Cfg &in) {
    row.cfg = in.cfg;
    row.suffix = in.suffix;
    row.comment = in.comment;
}

// --- atf_ci.FCitest.base.CopyOut
// Copy fields out of row
void atf_ci::citest_CopyOut(atf_ci::FCitest &row, atfdb::Citest &out) {
    out.citest = row.citest;
    out.needenv = row.needenv;
    out.cijob = row.cijob;
    out.comment = row.comment;
}

// --- atf_ci.FCitest.base.CopyIn
// Copy fields in to row
void atf_ci::citest_CopyIn(atf_ci::FCitest &row, atfdb::Citest &in) {
    row.citest = in.citest;
    row.needenv = in.needenv;
    row.cijob = in.cijob;
    row.comment = in.comment;
}

// --- atf_ci.FCitest..Uninit
void atf_ci::FCitest_Uninit(atf_ci::FCitest& citest) {
    atf_ci::FCitest &row = citest; (void)row;
    ind_citest_Remove(row); // remove citest from index ind_citest
}

// --- atf_ci.trace..Print
// print string representation of atf_ci::trace to string LHS, no header -- cprint:atf_ci.trace.String
void atf_ci::trace_Print(atf_ci::trace & row, algo::cstring &str) {
    algo::tempstr temp;
    str << "atf_ci.trace";
    (void)row;//only to avoid -Wunused-parameter
}

// --- atf_ci.FDb._db.MainArgs
// Main function
void atf_ci::MainArgs(int argc, char **argv) {
    Argtuple argtuple;
    Argtuple_ReadArgv(argtuple, argc,argv,atf_ci_syntax, atf_ci_help);
    vrfy(atf_ci_ReadTupleMaybe(atf_ci::_db.cmdline, argtuple.tuple),"where:read_cmdline");
    vrfy(atf_ci::LoadTuplesMaybe(atf_ci::_db.cmdline.in)
    ,tempstr()<<"where:load_input  "<<algo_lib::DetachBadTags());
    atf_ci::Main(); // call through to user-defined main
}

// --- atf_ci.FDb._db.MainLoop
// Main loop.
void atf_ci::MainLoop() {
    algo::SchedTime time(algo::get_cycles());
    algo_lib::_db.clock          = time;
    do {
        algo_lib::_db.next_loop.value = algo_lib::_db.limit;
        atf_ci::Steps();
    } while (algo_lib::_db.next_loop < algo_lib::_db.limit);
}

// --- atf_ci.FDb._db.Step
// Main step
void atf_ci::Step() {
}

// --- atf_ci.FDb._db.InitReflection
// Load statically available data into tables, register tables and database.
static void atf_ci::InitReflection() {
    algo_lib::imdb_InsertMaybe(algo::Imdb("atf_ci", atf_ci::InsertStrptrMaybe, NULL, atf_ci::MainLoop, NULL, algo::Comment()));

    algo::Imtable t_trace;
    t_trace.imtable         = "atf_ci.trace";
    t_trace.ssimfile        = "";
    t_trace.size            = sizeof(atf_ci::trace);
    t_trace.comment.value   = "";
    t_trace.c_RowidFind     = trace_RowidFind;
    t_trace.NItems          = trace_N;
    t_trace.Print           = (algo::ImrowPrintFcn)atf_ci::trace_Print;
    algo_lib::imtable_InsertMaybe(t_trace);


    // -- load signatures of existing dispatches --
    algo_lib::InsertStrptrMaybe("dmmeta.Dispsigcheck  dispsig:'atf_ci.Input'  signature:'d2065a16f956148bda6adce3f182fee068c7f400'");
}

// --- atf_ci.FDb._db.StaticCheck
void atf_ci::StaticCheck() {
    algo_assert(sizeof(atf_ci::citest_step_hook) == 8); // csize:atf_ci.citest_step_hook
    algo_assert(_offset_of(atf_ci::FieldId, value) + sizeof(((atf_ci::FieldId*)0)->value) == sizeof(atf_ci::FieldId));
}

// --- atf_ci.FDb._db.InsertStrptrMaybe
// Parse strptr into known type and add to database.
// Return value is true unless an error occurs. If return value is false, algo_lib::_db.errtext has error text
bool atf_ci::InsertStrptrMaybe(algo::strptr str) {
    bool retval = true;
    atf_ci::TableId table_id(-1);
    value_SetStrptrMaybe(table_id, algo::GetTypeTag(str));
    switch (value_GetEnum(table_id)) {
        case atf_ci_TableId_dmmeta_Ssimfile: { // finput:atf_ci.FDb.ssimfile
            dmmeta::Ssimfile elem;
            retval = dmmeta::Ssimfile_ReadStrptrMaybe(elem, str);
            retval = retval && ssimfile_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Scriptfile: { // finput:atf_ci.FDb.scriptfile
            dev::Scriptfile elem;
            retval = dev::Scriptfile_ReadStrptrMaybe(elem, str);
            retval = retval && scriptfile_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dmmeta_Ns: { // finput:atf_ci.FDb.ns
            dmmeta::Ns elem;
            retval = dmmeta::Ns_ReadStrptrMaybe(elem, str);
            retval = retval && ns_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Readme: { // finput:atf_ci.FDb.readme
            dev::Readme elem;
            retval = dev::Readme_ReadStrptrMaybe(elem, str);
            retval = retval && readme_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Builddir: { // finput:atf_ci.FDb.builddir
            dev::Builddir elem;
            retval = dev::Builddir_ReadStrptrMaybe(elem, str);
            retval = retval && builddir_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Cfg: { // finput:atf_ci.FDb.cfg
            dev::Cfg elem;
            retval = dev::Cfg_ReadStrptrMaybe(elem, str);
            retval = retval && cfg_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Gitfile: { // finput:atf_ci.FDb.gitfile
            dev::Gitfile elem;
            retval = dev::Gitfile_ReadStrptrMaybe(elem, str);
            retval = retval && gitfile_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Noindent: { // finput:atf_ci.FDb.noindent
            dev::Noindent elem;
            retval = dev::Noindent_ReadStrptrMaybe(elem, str);
            retval = retval && noindent_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Targsrc: { // finput:atf_ci.FDb.targsrc
            dev::Targsrc elem;
            retval = dev::Targsrc_ReadStrptrMaybe(elem, str);
            retval = retval && targsrc_InputMaybe(elem);
            break;
        }
        case atf_ci_TableId_dev_Msgfile: { // finput:atf_ci.FDb.msgfile
            dev::Msgfile elem;
            retval = dev::Msgfile_ReadStrptrMaybe(elem, str);
            retval = retval && msgfile_InputMaybe(elem);
            break;
        }
        default:
        retval = algo_lib::InsertStrptrMaybe(str);
        break;
    } //switch
    if (!retval) {
        algo_lib::NoteInsertErr(str); // increment error counter
    }
    return retval;
}

// --- atf_ci.FDb._db.LoadTuplesMaybe
// Load all finputs from given directory.
bool atf_ci::LoadTuplesMaybe(algo::strptr root) {
    bool retval = true;
    static const char *ssimfiles[] = {
        "dev.builddir", "dev.cfg", "dev.gitfile", "dev.msgfile"
        , "dev.noindent", "dmmeta.ns", "dev.readme", "dev.scriptfile"
        , "dmmeta.ssimfile", "dev.targsrc"
        , NULL};
        retval = algo_lib::DoLoadTuples(root, atf_ci::InsertStrptrMaybe, ssimfiles, true);
        return retval;
}

// --- atf_ci.FDb._db.LoadSsimfileMaybe
// Load specified ssimfile.
bool atf_ci::LoadSsimfileMaybe(algo::strptr fname) {
    bool retval = true;
    if (FileQ(fname)) {
        retval = algo_lib::LoadTuplesFile(fname, atf_ci::InsertStrptrMaybe, true);
    }
    return retval;
}

// --- atf_ci.FDb._db.Steps
// Calls Step function of dependencies
void atf_ci::Steps() {
    algo_lib::Step(); // dependent namespace specified via (dev.targdep)
}

// --- atf_ci.FDb._db.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::_db_XrefMaybe() {
    bool retval = true;
    return retval;
}

// --- atf_ci.FDb.citest.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FCitest& atf_ci::citest_Alloc() {
    atf_ci::FCitest* row = citest_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.citest  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.citest.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FCitest* atf_ci::citest_AllocMaybe() {
    atf_ci::FCitest *row = (atf_ci::FCitest*)citest_AllocMem();
    if (row) {
        new (row) atf_ci::FCitest; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.citest.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FCitest* atf_ci::citest_InsertMaybe(const atfdb::Citest &value) {
    atf_ci::FCitest *row = &citest_Alloc(); // if out of memory, process dies. if input error, return NULL.
    citest_CopyIn(*row,const_cast<atfdb::Citest&>(value));
    bool ok = citest_XrefMaybe(*row); // this may return false
    if (!ok) {
        citest_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.citest.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::citest_AllocMem() {
    u64 new_nelems     = _db.citest_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FCitest*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.citest_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FCitest*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FCitest) * (u64(1)<<bsr));
            _db.citest_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.citest_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.citest.RemoveAll
// Remove all elements from Lary
void atf_ci::citest_RemoveAll() {
    for (u64 n = _db.citest_n; n>0; ) {
        n--;
        citest_qFind(u64(n)).~FCitest(); // destroy last element
        _db.citest_n = i32(n);
    }
}

// --- atf_ci.FDb.citest.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::citest_RemoveLast() {
    u64 n = _db.citest_n;
    if (n > 0) {
        n -= 1;
        citest_qFind(u64(n)).~FCitest();
        _db.citest_n = i32(n);
    }
}

// --- atf_ci.FDb.citest.LoadStatic
static void atf_ci::citest_LoadStatic() {
    static struct _t {
        const char *s;
        void (*step)();
    } data[] = {
        { "atfdb.citest  citest:checkclean  needenv:N  cijob:normalize  comment:\"Check that no files are modified\"", atf_ci::citest_checkclean }
        ,{ "atfdb.citest  citest:outfile  needenv:N  cijob:normalize  comment:\"\"", atf_ci::citest_outfile }
        ,{ "atfdb.citest  citest:atf_amc  needenv:N  cijob:normalize  comment:\"Test amc (run atf_amc)\"", atf_ci::citest_atf_amc }
        ,{ "atfdb.citest  citest:gitfile  needenv:N  cijob:normalize  comment:\"Update gitfile tables by scanning filesystem\"", atf_ci::citest_gitfile }
        ,{ "atfdb.citest  citest:normalize_acr  needenv:N  cijob:normalize  comment:\"Read ssim databases into memory and write back\"", atf_ci::citest_normalize_acr }
        ,{ "atfdb.citest  citest:src_lim  needenv:N  cijob:normalize  comment:\"Source code police\"", atf_ci::citest_src_lim }
        ,{ "atfdb.citest  citest:amc  needenv:N  cijob:normalize  comment:\"Run amc\"", atf_ci::citest_amc }
        ,{ "atfdb.citest  citest:bootstrap  needenv:N  cijob:normalize  comment:\"Re-generate bootstrap files\"", atf_ci::citest_bootstrap }
        ,{ "atfdb.citest  citest:shebang  needenv:N  cijob:normalize  comment:\"\"", atf_ci::citest_shebang }
        ,{ "atfdb.citest  citest:encoding  needenv:N  cijob:normalize  comment:\"Check Encoding of h/cpp files\"", atf_ci::citest_encoding }
        ,{ "atfdb.citest  citest:inline_readme  needenv:N  cijob:normalize  comment:\"Re-generate inline blocks for txt/*.md files\"", atf_ci::citest_inline_readme }
        ,{ "atfdb.citest  citest:readme  needenv:N  cijob:normalize  comment:\"Re-generate README.md table of contents\"", atf_ci::citest_readme }
        ,{ "atfdb.citest  citest:copyright  needenv:N  cijob:normalize  comment:\"Update copyrights in source files\"", atf_ci::citest_copyright }
        ,{ "atfdb.citest  citest:iffy_src  needenv:N  cijob:normalize  comment:\"Check for iffy source constructs with src_func\"", atf_ci::citest_iffy_src }
        ,{ "atfdb.citest  citest:stray_gen  needenv:N  cijob:normalize  comment:\"*/gen/* file that doesn't appear to be generated by amc\"", atf_ci::citest_stray_gen }
        ,{ "atfdb.citest  citest:tempcode  needenv:N  cijob:normalize  comment:\"Check for temp code inserted for testing only\"", atf_ci::citest_tempcode }
        ,{ "atfdb.citest  citest:lineendings  needenv:N  cijob:normalize  comment:\"Correct windows-style line endings in known text files\"", atf_ci::citest_lineendings }
        ,{ "atfdb.citest  citest:indent_script  needenv:N  cijob:normalize  comment:\"Indent any bash script file\"", atf_ci::citest_indent_script }
        ,{ "atfdb.citest  citest:comptest  needenv:N  cijob:normalize  comment:\"Rewrite/normalize component tests\"", atf_ci::citest_comptest }
        ,{ "atfdb.citest  citest:cppcheck  needenv:N  cijob:normalize  comment:\"Cppcheck static code analysis\"", atf_ci::citest_cppcheck }
        ,{ "atfdb.citest  citest:bintests  needenv:N  cijob:comp  comment:\"Run bin/test-* scripts\"", atf_ci::citest_bintests }
        ,{ "atfdb.citest  citest:indent_srcfile  needenv:N  cijob:normalize  comment:\"Indent any source files modified in last commit\"", atf_ci::citest_indent_srcfile }
        ,{ "atfdb.citest  citest:normalize_amc_vis  needenv:N  cijob:normalize  comment:\"Check that amc_vis doesn't see any circular dependencies\"", atf_ci::citest_normalize_amc_vis }
        ,{ "atfdb.citest  citest:ssimfile  needenv:N  cijob:normalize  comment:\"Check for .ssim files with no corresponding ssimfile entry\"", atf_ci::citest_ssimfile }
        ,{ "atfdb.citest  citest:normalize_acr_my  needenv:N  cijob:normalize  comment:\"Round trip ssim databases through MariaDB and back\"", atf_ci::citest_normalize_acr_my }
        ,{ "atfdb.citest  citest:atf_unit  needenv:N  cijob:comp  comment:\"Run unit tests\"", atf_ci::citest_atf_unit }
        ,{ "atfdb.citest  citest:atf_comp  needenv:Y  cijob:comp  comment:\"Run component tests\"", atf_ci::citest_atf_comp }
        ,{ "atfdb.citest  citest:atf_comp_cov  needenv:Y  cijob:comp  comment:\"Check component test coverage\"", atf_ci::citest_atf_comp_cov }
        ,{ "atfdb.citest  citest:atf_comp_mem  needenv:Y  cijob:memcheck  comment:\"Run component tests in memcheck mode (slow)\"", atf_ci::citest_atf_comp_mem }
        ,{ "atfdb.citest  citest:acr_ed_ssimfile  needenv:N  cijob:comp  comment:\"Create a new ssimfile\"", atf_ci::citest_acr_ed_ssimfile }
        ,{ "atfdb.citest  citest:acr_ed_ssimdb  needenv:N  cijob:comp  comment:\"Create a new ssimdb\"", atf_ci::citest_acr_ed_ssimdb }
        ,{ "atfdb.citest  citest:acr_ed_target  needenv:N  cijob:comp  comment:\"Takes a while - do it last\"", atf_ci::citest_acr_ed_target }
        ,{NULL, NULL}
    };
    (void)data;
    atfdb::Citest citest;
    for (int i=0; data[i].s; i++) {
        (void)atfdb::Citest_ReadStrptrMaybe(citest, algo::strptr(data[i].s));
        atf_ci::FCitest *elem = citest_InsertMaybe(citest);
        vrfy(elem, tempstr("atf_ci.static_insert_fatal_error")
        << Keyval("tuple",algo::strptr(data[i].s))
        << Keyval("comment",algo_lib::DetachBadTags()));
        elem->step = data[i].step;
    }
}

// --- atf_ci.FDb.citest.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::citest_XrefMaybe(atf_ci::FCitest &row) {
    bool retval = true;
    (void)row;
    // insert citest into index ind_citest
    if (true) { // user-defined insert condition
        bool success = ind_citest_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FDb.ind_citest"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ssimfile.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FSsimfile& atf_ci::ssimfile_Alloc() {
    atf_ci::FSsimfile* row = ssimfile_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.ssimfile  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.ssimfile.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FSsimfile* atf_ci::ssimfile_AllocMaybe() {
    atf_ci::FSsimfile *row = (atf_ci::FSsimfile*)ssimfile_AllocMem();
    if (row) {
        new (row) atf_ci::FSsimfile; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.ssimfile.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FSsimfile* atf_ci::ssimfile_InsertMaybe(const dmmeta::Ssimfile &value) {
    atf_ci::FSsimfile *row = &ssimfile_Alloc(); // if out of memory, process dies. if input error, return NULL.
    ssimfile_CopyIn(*row,const_cast<dmmeta::Ssimfile&>(value));
    bool ok = ssimfile_XrefMaybe(*row); // this may return false
    if (!ok) {
        ssimfile_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.ssimfile.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::ssimfile_AllocMem() {
    u64 new_nelems     = _db.ssimfile_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FSsimfile*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.ssimfile_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FSsimfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FSsimfile) * (u64(1)<<bsr));
            _db.ssimfile_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.ssimfile_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.ssimfile.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::ssimfile_RemoveLast() {
    u64 n = _db.ssimfile_n;
    if (n > 0) {
        n -= 1;
        ssimfile_qFind(u64(n)).~FSsimfile();
        _db.ssimfile_n = i32(n);
    }
}

// --- atf_ci.FDb.ssimfile.InputMaybe
static bool atf_ci::ssimfile_InputMaybe(dmmeta::Ssimfile &elem) {
    bool retval = true;
    retval = ssimfile_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.ssimfile.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::ssimfile_XrefMaybe(atf_ci::FSsimfile &row) {
    bool retval = true;
    (void)row;
    // insert ssimfile into index ind_ssimfile
    if (true) { // user-defined insert condition
        bool success = ind_ssimfile_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FDb.ind_ssimfile"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_ssimfile.Find
// Find row by key. Return NULL if not found.
atf_ci::FSsimfile* atf_ci::ind_ssimfile_Find(const algo::strptr& key) {
    u32 index = algo::Smallstr50_Hash(0, key) & (_db.ind_ssimfile_buckets_n - 1);
    atf_ci::FSsimfile* *e = &_db.ind_ssimfile_buckets_elems[index];
    atf_ci::FSsimfile* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).ssimfile == key;
        if (done) break;
        e         = &ret->ind_ssimfile_next;
    } while (true);
    return ret;
}

// --- atf_ci.FDb.ind_ssimfile.FindX
// Look up row by key and return reference. Throw exception if not found
atf_ci::FSsimfile& atf_ci::ind_ssimfile_FindX(const algo::strptr& key) {
    atf_ci::FSsimfile* ret = ind_ssimfile_Find(key);
    vrfy(ret, tempstr() << "atf_ci.key_error  table:ind_ssimfile  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- atf_ci.FDb.ind_ssimfile.GetOrCreate
// Find row by key. If not found, create and x-reference a new row with with this key.
atf_ci::FSsimfile& atf_ci::ind_ssimfile_GetOrCreate(const algo::strptr& key) {
    atf_ci::FSsimfile* ret = ind_ssimfile_Find(key);
    if (!ret) { //  if memory alloc fails, process dies; if insert fails, function returns NULL.
        ret         = &ssimfile_Alloc();
        (*ret).ssimfile = key;
        bool good = ssimfile_XrefMaybe(*ret);
        if (!good) {
            ssimfile_RemoveLast(); // delete offending row, any existing xrefs are cleared
            ret = NULL;
        }
    }
    vrfy(ret, tempstr() << "atf_ci.create_error  table:ind_ssimfile  key:'"<<key<<"'  comment:'bad xref'");
    return *ret;
}

// --- atf_ci.FDb.ind_ssimfile.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool atf_ci::ind_ssimfile_InsertMaybe(atf_ci::FSsimfile& row) {
    ind_ssimfile_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_ssimfile_next == (atf_ci::FSsimfile*)-1)) {// check if in hash already
        u32 index = algo::Smallstr50_Hash(0, row.ssimfile) & (_db.ind_ssimfile_buckets_n - 1);
        atf_ci::FSsimfile* *prev = &_db.ind_ssimfile_buckets_elems[index];
        do {
            atf_ci::FSsimfile* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).ssimfile == row.ssimfile) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_ssimfile_next;
        } while (true);
        if (retval) {
            row.ind_ssimfile_next = *prev;
            _db.ind_ssimfile_n++;
            *prev = &row;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_ssimfile.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void atf_ci::ind_ssimfile_Remove(atf_ci::FSsimfile& row) {
    if (LIKELY(row.ind_ssimfile_next != (atf_ci::FSsimfile*)-1)) {// check if in hash already
        u32 index = algo::Smallstr50_Hash(0, row.ssimfile) & (_db.ind_ssimfile_buckets_n - 1);
        atf_ci::FSsimfile* *prev = &_db.ind_ssimfile_buckets_elems[index]; // addr of pointer to current element
        while (atf_ci::FSsimfile *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_ssimfile_next; // unlink (singly linked list)
                _db.ind_ssimfile_n--;
                row.ind_ssimfile_next = (atf_ci::FSsimfile*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_ssimfile_next;
        }
    }
}

// --- atf_ci.FDb.ind_ssimfile.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void atf_ci::ind_ssimfile_Reserve(int n) {
    u32 old_nbuckets = _db.ind_ssimfile_buckets_n;
    u32 new_nelems   = _db.ind_ssimfile_n + n;
    // # of elements has to be roughly equal to the number of buckets
    if (new_nelems > old_nbuckets) {
        int new_nbuckets = i32_Max(algo::BumpToPow2(new_nelems), u32(4));
        u32 old_size = old_nbuckets * sizeof(atf_ci::FSsimfile*);
        u32 new_size = new_nbuckets * sizeof(atf_ci::FSsimfile*);
        // allocate new array. we don't use Realloc since copying is not needed and factor of 2 probably
        // means new memory will have to be allocated anyway
        atf_ci::FSsimfile* *new_buckets = (atf_ci::FSsimfile**)algo_lib::malloc_AllocMem(new_size);
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("atf_ci.out_of_memory  field:atf_ci.FDb.ind_ssimfile");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_ssimfile_buckets_n; i++) {
            atf_ci::FSsimfile* elem = _db.ind_ssimfile_buckets_elems[i];
            while (elem) {
                atf_ci::FSsimfile &row        = *elem;
                atf_ci::FSsimfile* next       = row.ind_ssimfile_next;
                u32 index          = algo::Smallstr50_Hash(0, row.ssimfile) & (new_nbuckets-1);
                row.ind_ssimfile_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_ssimfile_buckets_elems, old_size);
        _db.ind_ssimfile_buckets_elems = new_buckets;
        _db.ind_ssimfile_buckets_n = new_nbuckets;
    }
}

// --- atf_ci.FDb.scriptfile.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FScriptfile& atf_ci::scriptfile_Alloc() {
    atf_ci::FScriptfile* row = scriptfile_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.scriptfile  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.scriptfile.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FScriptfile* atf_ci::scriptfile_AllocMaybe() {
    atf_ci::FScriptfile *row = (atf_ci::FScriptfile*)scriptfile_AllocMem();
    if (row) {
        new (row) atf_ci::FScriptfile; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.scriptfile.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FScriptfile* atf_ci::scriptfile_InsertMaybe(const dev::Scriptfile &value) {
    atf_ci::FScriptfile *row = &scriptfile_Alloc(); // if out of memory, process dies. if input error, return NULL.
    scriptfile_CopyIn(*row,const_cast<dev::Scriptfile&>(value));
    bool ok = scriptfile_XrefMaybe(*row); // this may return false
    if (!ok) {
        scriptfile_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.scriptfile.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::scriptfile_AllocMem() {
    u64 new_nelems     = _db.scriptfile_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FScriptfile*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.scriptfile_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FScriptfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FScriptfile) * (u64(1)<<bsr));
            _db.scriptfile_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.scriptfile_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.scriptfile.RemoveAll
// Remove all elements from Lary
void atf_ci::scriptfile_RemoveAll() {
    for (u64 n = _db.scriptfile_n; n>0; ) {
        n--;
        scriptfile_qFind(u64(n)).~FScriptfile(); // destroy last element
        _db.scriptfile_n = i32(n);
    }
}

// --- atf_ci.FDb.scriptfile.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::scriptfile_RemoveLast() {
    u64 n = _db.scriptfile_n;
    if (n > 0) {
        n -= 1;
        scriptfile_qFind(u64(n)).~FScriptfile();
        _db.scriptfile_n = i32(n);
    }
}

// --- atf_ci.FDb.scriptfile.InputMaybe
static bool atf_ci::scriptfile_InputMaybe(dev::Scriptfile &elem) {
    bool retval = true;
    retval = scriptfile_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.scriptfile.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::scriptfile_XrefMaybe(atf_ci::FScriptfile &row) {
    bool retval = true;
    (void)row;
    // insert scriptfile into index ind_scriptfile
    if (true) { // user-defined insert condition
        bool success = ind_scriptfile_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FDb.ind_scriptfile"; // check for duplicate key
            return false;
        }
    }
    atf_ci::FGitfile* p_gitfile = atf_ci::ind_gitfile_Find(row.gitfile);
    if (UNLIKELY(!p_gitfile)) {
        algo_lib::ResetErrtext() << "atf_ci.bad_xref  index:atf_ci.FDb.ind_gitfile" << Keyval("key", row.gitfile);
        return false;
    }
    // insert scriptfile into index c_scriptfile
    if (true) { // user-defined insert condition
        bool success = c_scriptfile_InsertMaybe(*p_gitfile, row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FGitfile.c_scriptfile"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_scriptfile.Find
// Find row by key. Return NULL if not found.
atf_ci::FScriptfile* atf_ci::ind_scriptfile_Find(const algo::strptr& key) {
    u32 index = algo::Smallstr200_Hash(0, key) & (_db.ind_scriptfile_buckets_n - 1);
    atf_ci::FScriptfile* *e = &_db.ind_scriptfile_buckets_elems[index];
    atf_ci::FScriptfile* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).gitfile == key;
        if (done) break;
        e         = &ret->ind_scriptfile_next;
    } while (true);
    return ret;
}

// --- atf_ci.FDb.ind_scriptfile.FindX
// Look up row by key and return reference. Throw exception if not found
atf_ci::FScriptfile& atf_ci::ind_scriptfile_FindX(const algo::strptr& key) {
    atf_ci::FScriptfile* ret = ind_scriptfile_Find(key);
    vrfy(ret, tempstr() << "atf_ci.key_error  table:ind_scriptfile  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- atf_ci.FDb.ind_scriptfile.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool atf_ci::ind_scriptfile_InsertMaybe(atf_ci::FScriptfile& row) {
    ind_scriptfile_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_scriptfile_next == (atf_ci::FScriptfile*)-1)) {// check if in hash already
        u32 index = algo::Smallstr200_Hash(0, row.gitfile) & (_db.ind_scriptfile_buckets_n - 1);
        atf_ci::FScriptfile* *prev = &_db.ind_scriptfile_buckets_elems[index];
        do {
            atf_ci::FScriptfile* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).gitfile == row.gitfile) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_scriptfile_next;
        } while (true);
        if (retval) {
            row.ind_scriptfile_next = *prev;
            _db.ind_scriptfile_n++;
            *prev = &row;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_scriptfile.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void atf_ci::ind_scriptfile_Remove(atf_ci::FScriptfile& row) {
    if (LIKELY(row.ind_scriptfile_next != (atf_ci::FScriptfile*)-1)) {// check if in hash already
        u32 index = algo::Smallstr200_Hash(0, row.gitfile) & (_db.ind_scriptfile_buckets_n - 1);
        atf_ci::FScriptfile* *prev = &_db.ind_scriptfile_buckets_elems[index]; // addr of pointer to current element
        while (atf_ci::FScriptfile *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_scriptfile_next; // unlink (singly linked list)
                _db.ind_scriptfile_n--;
                row.ind_scriptfile_next = (atf_ci::FScriptfile*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_scriptfile_next;
        }
    }
}

// --- atf_ci.FDb.ind_scriptfile.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void atf_ci::ind_scriptfile_Reserve(int n) {
    u32 old_nbuckets = _db.ind_scriptfile_buckets_n;
    u32 new_nelems   = _db.ind_scriptfile_n + n;
    // # of elements has to be roughly equal to the number of buckets
    if (new_nelems > old_nbuckets) {
        int new_nbuckets = i32_Max(algo::BumpToPow2(new_nelems), u32(4));
        u32 old_size = old_nbuckets * sizeof(atf_ci::FScriptfile*);
        u32 new_size = new_nbuckets * sizeof(atf_ci::FScriptfile*);
        // allocate new array. we don't use Realloc since copying is not needed and factor of 2 probably
        // means new memory will have to be allocated anyway
        atf_ci::FScriptfile* *new_buckets = (atf_ci::FScriptfile**)algo_lib::malloc_AllocMem(new_size);
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("atf_ci.out_of_memory  field:atf_ci.FDb.ind_scriptfile");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_scriptfile_buckets_n; i++) {
            atf_ci::FScriptfile* elem = _db.ind_scriptfile_buckets_elems[i];
            while (elem) {
                atf_ci::FScriptfile &row        = *elem;
                atf_ci::FScriptfile* next       = row.ind_scriptfile_next;
                u32 index          = algo::Smallstr200_Hash(0, row.gitfile) & (new_nbuckets-1);
                row.ind_scriptfile_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_scriptfile_buckets_elems, old_size);
        _db.ind_scriptfile_buckets_elems = new_buckets;
        _db.ind_scriptfile_buckets_n = new_nbuckets;
    }
}

// --- atf_ci.FDb.ns.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FNs& atf_ci::ns_Alloc() {
    atf_ci::FNs* row = ns_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.ns  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.ns.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FNs* atf_ci::ns_AllocMaybe() {
    atf_ci::FNs *row = (atf_ci::FNs*)ns_AllocMem();
    if (row) {
        new (row) atf_ci::FNs; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.ns.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FNs* atf_ci::ns_InsertMaybe(const dmmeta::Ns &value) {
    atf_ci::FNs *row = &ns_Alloc(); // if out of memory, process dies. if input error, return NULL.
    ns_CopyIn(*row,const_cast<dmmeta::Ns&>(value));
    bool ok = ns_XrefMaybe(*row); // this may return false
    if (!ok) {
        ns_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.ns.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::ns_AllocMem() {
    u64 new_nelems     = _db.ns_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FNs*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.ns_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FNs*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FNs) * (u64(1)<<bsr));
            _db.ns_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.ns_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.ns.RemoveAll
// Remove all elements from Lary
void atf_ci::ns_RemoveAll() {
    for (u64 n = _db.ns_n; n>0; ) {
        n--;
        ns_qFind(u64(n)).~FNs(); // destroy last element
        _db.ns_n = i32(n);
    }
}

// --- atf_ci.FDb.ns.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::ns_RemoveLast() {
    u64 n = _db.ns_n;
    if (n > 0) {
        n -= 1;
        ns_qFind(u64(n)).~FNs();
        _db.ns_n = i32(n);
    }
}

// --- atf_ci.FDb.ns.InputMaybe
static bool atf_ci::ns_InputMaybe(dmmeta::Ns &elem) {
    bool retval = true;
    retval = ns_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.ns.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::ns_XrefMaybe(atf_ci::FNs &row) {
    bool retval = true;
    (void)row;
    // insert ns into index ind_ns
    if (true) { // user-defined insert condition
        bool success = ind_ns_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FDb.ind_ns"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_ns.Find
// Find row by key. Return NULL if not found.
atf_ci::FNs* atf_ci::ind_ns_Find(const algo::strptr& key) {
    u32 index = algo::Smallstr16_Hash(0, key) & (_db.ind_ns_buckets_n - 1);
    atf_ci::FNs* *e = &_db.ind_ns_buckets_elems[index];
    atf_ci::FNs* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).ns == key;
        if (done) break;
        e         = &ret->ind_ns_next;
    } while (true);
    return ret;
}

// --- atf_ci.FDb.ind_ns.FindX
// Look up row by key and return reference. Throw exception if not found
atf_ci::FNs& atf_ci::ind_ns_FindX(const algo::strptr& key) {
    atf_ci::FNs* ret = ind_ns_Find(key);
    vrfy(ret, tempstr() << "atf_ci.key_error  table:ind_ns  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- atf_ci.FDb.ind_ns.GetOrCreate
// Find row by key. If not found, create and x-reference a new row with with this key.
atf_ci::FNs& atf_ci::ind_ns_GetOrCreate(const algo::strptr& key) {
    atf_ci::FNs* ret = ind_ns_Find(key);
    if (!ret) { //  if memory alloc fails, process dies; if insert fails, function returns NULL.
        ret         = &ns_Alloc();
        (*ret).ns = key;
        bool good = ns_XrefMaybe(*ret);
        if (!good) {
            ns_RemoveLast(); // delete offending row, any existing xrefs are cleared
            ret = NULL;
        }
    }
    vrfy(ret, tempstr() << "atf_ci.create_error  table:ind_ns  key:'"<<key<<"'  comment:'bad xref'");
    return *ret;
}

// --- atf_ci.FDb.ind_ns.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool atf_ci::ind_ns_InsertMaybe(atf_ci::FNs& row) {
    ind_ns_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_ns_next == (atf_ci::FNs*)-1)) {// check if in hash already
        u32 index = algo::Smallstr16_Hash(0, row.ns) & (_db.ind_ns_buckets_n - 1);
        atf_ci::FNs* *prev = &_db.ind_ns_buckets_elems[index];
        do {
            atf_ci::FNs* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).ns == row.ns) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_ns_next;
        } while (true);
        if (retval) {
            row.ind_ns_next = *prev;
            _db.ind_ns_n++;
            *prev = &row;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_ns.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void atf_ci::ind_ns_Remove(atf_ci::FNs& row) {
    if (LIKELY(row.ind_ns_next != (atf_ci::FNs*)-1)) {// check if in hash already
        u32 index = algo::Smallstr16_Hash(0, row.ns) & (_db.ind_ns_buckets_n - 1);
        atf_ci::FNs* *prev = &_db.ind_ns_buckets_elems[index]; // addr of pointer to current element
        while (atf_ci::FNs *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_ns_next; // unlink (singly linked list)
                _db.ind_ns_n--;
                row.ind_ns_next = (atf_ci::FNs*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_ns_next;
        }
    }
}

// --- atf_ci.FDb.ind_ns.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void atf_ci::ind_ns_Reserve(int n) {
    u32 old_nbuckets = _db.ind_ns_buckets_n;
    u32 new_nelems   = _db.ind_ns_n + n;
    // # of elements has to be roughly equal to the number of buckets
    if (new_nelems > old_nbuckets) {
        int new_nbuckets = i32_Max(algo::BumpToPow2(new_nelems), u32(4));
        u32 old_size = old_nbuckets * sizeof(atf_ci::FNs*);
        u32 new_size = new_nbuckets * sizeof(atf_ci::FNs*);
        // allocate new array. we don't use Realloc since copying is not needed and factor of 2 probably
        // means new memory will have to be allocated anyway
        atf_ci::FNs* *new_buckets = (atf_ci::FNs**)algo_lib::malloc_AllocMem(new_size);
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("atf_ci.out_of_memory  field:atf_ci.FDb.ind_ns");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_ns_buckets_n; i++) {
            atf_ci::FNs* elem = _db.ind_ns_buckets_elems[i];
            while (elem) {
                atf_ci::FNs &row        = *elem;
                atf_ci::FNs* next       = row.ind_ns_next;
                u32 index          = algo::Smallstr16_Hash(0, row.ns) & (new_nbuckets-1);
                row.ind_ns_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_ns_buckets_elems, old_size);
        _db.ind_ns_buckets_elems = new_buckets;
        _db.ind_ns_buckets_n = new_nbuckets;
    }
}

// --- atf_ci.FDb.readme.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FReadme& atf_ci::readme_Alloc() {
    atf_ci::FReadme* row = readme_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.readme  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.readme.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FReadme* atf_ci::readme_AllocMaybe() {
    atf_ci::FReadme *row = (atf_ci::FReadme*)readme_AllocMem();
    if (row) {
        new (row) atf_ci::FReadme; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.readme.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FReadme* atf_ci::readme_InsertMaybe(const dev::Readme &value) {
    atf_ci::FReadme *row = &readme_Alloc(); // if out of memory, process dies. if input error, return NULL.
    readme_CopyIn(*row,const_cast<dev::Readme&>(value));
    bool ok = readme_XrefMaybe(*row); // this may return false
    if (!ok) {
        readme_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.readme.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::readme_AllocMem() {
    u64 new_nelems     = _db.readme_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FReadme*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.readme_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FReadme*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FReadme) * (u64(1)<<bsr));
            _db.readme_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.readme_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.readme.RemoveAll
// Remove all elements from Lary
void atf_ci::readme_RemoveAll() {
    for (u64 n = _db.readme_n; n>0; ) {
        n--;
        readme_qFind(u64(n)).~FReadme(); // destroy last element
        _db.readme_n = i32(n);
    }
}

// --- atf_ci.FDb.readme.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::readme_RemoveLast() {
    u64 n = _db.readme_n;
    if (n > 0) {
        n -= 1;
        readme_qFind(u64(n)).~FReadme();
        _db.readme_n = i32(n);
    }
}

// --- atf_ci.FDb.readme.InputMaybe
static bool atf_ci::readme_InputMaybe(dev::Readme &elem) {
    bool retval = true;
    retval = readme_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.readme.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::readme_XrefMaybe(atf_ci::FReadme &row) {
    bool retval = true;
    (void)row;
    return retval;
}

// --- atf_ci.FDb.builddir.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FBuilddir& atf_ci::builddir_Alloc() {
    atf_ci::FBuilddir* row = builddir_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.builddir  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.builddir.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FBuilddir* atf_ci::builddir_AllocMaybe() {
    atf_ci::FBuilddir *row = (atf_ci::FBuilddir*)builddir_AllocMem();
    if (row) {
        new (row) atf_ci::FBuilddir; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.builddir.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FBuilddir* atf_ci::builddir_InsertMaybe(const dev::Builddir &value) {
    atf_ci::FBuilddir *row = &builddir_Alloc(); // if out of memory, process dies. if input error, return NULL.
    builddir_CopyIn(*row,const_cast<dev::Builddir&>(value));
    bool ok = builddir_XrefMaybe(*row); // this may return false
    if (!ok) {
        builddir_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.builddir.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::builddir_AllocMem() {
    u64 new_nelems     = _db.builddir_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FBuilddir*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.builddir_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FBuilddir*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FBuilddir) * (u64(1)<<bsr));
            _db.builddir_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.builddir_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.builddir.RemoveAll
// Remove all elements from Lary
void atf_ci::builddir_RemoveAll() {
    for (u64 n = _db.builddir_n; n>0; ) {
        n--;
        builddir_qFind(u64(n)).~FBuilddir(); // destroy last element
        _db.builddir_n = i32(n);
    }
}

// --- atf_ci.FDb.builddir.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::builddir_RemoveLast() {
    u64 n = _db.builddir_n;
    if (n > 0) {
        n -= 1;
        builddir_qFind(u64(n)).~FBuilddir();
        _db.builddir_n = i32(n);
    }
}

// --- atf_ci.FDb.builddir.InputMaybe
static bool atf_ci::builddir_InputMaybe(dev::Builddir &elem) {
    bool retval = true;
    retval = builddir_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.builddir.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::builddir_XrefMaybe(atf_ci::FBuilddir &row) {
    bool retval = true;
    (void)row;
    // insert builddir into index ind_builddir
    if (true) { // user-defined insert condition
        bool success = ind_builddir_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FDb.ind_builddir"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.cfg.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FCfg& atf_ci::cfg_Alloc() {
    atf_ci::FCfg* row = cfg_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.cfg  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.cfg.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FCfg* atf_ci::cfg_AllocMaybe() {
    atf_ci::FCfg *row = (atf_ci::FCfg*)cfg_AllocMem();
    if (row) {
        new (row) atf_ci::FCfg; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.cfg.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FCfg* atf_ci::cfg_InsertMaybe(const dev::Cfg &value) {
    atf_ci::FCfg *row = &cfg_Alloc(); // if out of memory, process dies. if input error, return NULL.
    cfg_CopyIn(*row,const_cast<dev::Cfg&>(value));
    bool ok = cfg_XrefMaybe(*row); // this may return false
    if (!ok) {
        cfg_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.cfg.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::cfg_AllocMem() {
    u64 new_nelems     = _db.cfg_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FCfg*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.cfg_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FCfg*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FCfg) * (u64(1)<<bsr));
            _db.cfg_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.cfg_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.cfg.RemoveAll
// Remove all elements from Lary
void atf_ci::cfg_RemoveAll() {
    for (u64 n = _db.cfg_n; n>0; ) {
        n--;
        cfg_qFind(u64(n)).~FCfg(); // destroy last element
        _db.cfg_n = i32(n);
    }
}

// --- atf_ci.FDb.cfg.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::cfg_RemoveLast() {
    u64 n = _db.cfg_n;
    if (n > 0) {
        n -= 1;
        cfg_qFind(u64(n)).~FCfg();
        _db.cfg_n = i32(n);
    }
}

// --- atf_ci.FDb.cfg.InputMaybe
static bool atf_ci::cfg_InputMaybe(dev::Cfg &elem) {
    bool retval = true;
    retval = cfg_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.cfg.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::cfg_XrefMaybe(atf_ci::FCfg &row) {
    bool retval = true;
    (void)row;
    return retval;
}

// --- atf_ci.FDb.ind_builddir.Find
// Find row by key. Return NULL if not found.
atf_ci::FBuilddir* atf_ci::ind_builddir_Find(const algo::strptr& key) {
    u32 index = algo::Smallstr50_Hash(0, key) & (_db.ind_builddir_buckets_n - 1);
    atf_ci::FBuilddir* *e = &_db.ind_builddir_buckets_elems[index];
    atf_ci::FBuilddir* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).builddir == key;
        if (done) break;
        e         = &ret->ind_builddir_next;
    } while (true);
    return ret;
}

// --- atf_ci.FDb.ind_builddir.FindX
// Look up row by key and return reference. Throw exception if not found
atf_ci::FBuilddir& atf_ci::ind_builddir_FindX(const algo::strptr& key) {
    atf_ci::FBuilddir* ret = ind_builddir_Find(key);
    vrfy(ret, tempstr() << "atf_ci.key_error  table:ind_builddir  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- atf_ci.FDb.ind_builddir.GetOrCreate
// Find row by key. If not found, create and x-reference a new row with with this key.
atf_ci::FBuilddir& atf_ci::ind_builddir_GetOrCreate(const algo::strptr& key) {
    atf_ci::FBuilddir* ret = ind_builddir_Find(key);
    if (!ret) { //  if memory alloc fails, process dies; if insert fails, function returns NULL.
        ret         = &builddir_Alloc();
        (*ret).builddir = key;
        bool good = builddir_XrefMaybe(*ret);
        if (!good) {
            builddir_RemoveLast(); // delete offending row, any existing xrefs are cleared
            ret = NULL;
        }
    }
    vrfy(ret, tempstr() << "atf_ci.create_error  table:ind_builddir  key:'"<<key<<"'  comment:'bad xref'");
    return *ret;
}

// --- atf_ci.FDb.ind_builddir.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool atf_ci::ind_builddir_InsertMaybe(atf_ci::FBuilddir& row) {
    ind_builddir_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_builddir_next == (atf_ci::FBuilddir*)-1)) {// check if in hash already
        u32 index = algo::Smallstr50_Hash(0, row.builddir) & (_db.ind_builddir_buckets_n - 1);
        atf_ci::FBuilddir* *prev = &_db.ind_builddir_buckets_elems[index];
        do {
            atf_ci::FBuilddir* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).builddir == row.builddir) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_builddir_next;
        } while (true);
        if (retval) {
            row.ind_builddir_next = *prev;
            _db.ind_builddir_n++;
            *prev = &row;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_builddir.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void atf_ci::ind_builddir_Remove(atf_ci::FBuilddir& row) {
    if (LIKELY(row.ind_builddir_next != (atf_ci::FBuilddir*)-1)) {// check if in hash already
        u32 index = algo::Smallstr50_Hash(0, row.builddir) & (_db.ind_builddir_buckets_n - 1);
        atf_ci::FBuilddir* *prev = &_db.ind_builddir_buckets_elems[index]; // addr of pointer to current element
        while (atf_ci::FBuilddir *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_builddir_next; // unlink (singly linked list)
                _db.ind_builddir_n--;
                row.ind_builddir_next = (atf_ci::FBuilddir*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_builddir_next;
        }
    }
}

// --- atf_ci.FDb.ind_builddir.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void atf_ci::ind_builddir_Reserve(int n) {
    u32 old_nbuckets = _db.ind_builddir_buckets_n;
    u32 new_nelems   = _db.ind_builddir_n + n;
    // # of elements has to be roughly equal to the number of buckets
    if (new_nelems > old_nbuckets) {
        int new_nbuckets = i32_Max(algo::BumpToPow2(new_nelems), u32(4));
        u32 old_size = old_nbuckets * sizeof(atf_ci::FBuilddir*);
        u32 new_size = new_nbuckets * sizeof(atf_ci::FBuilddir*);
        // allocate new array. we don't use Realloc since copying is not needed and factor of 2 probably
        // means new memory will have to be allocated anyway
        atf_ci::FBuilddir* *new_buckets = (atf_ci::FBuilddir**)algo_lib::malloc_AllocMem(new_size);
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("atf_ci.out_of_memory  field:atf_ci.FDb.ind_builddir");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_builddir_buckets_n; i++) {
            atf_ci::FBuilddir* elem = _db.ind_builddir_buckets_elems[i];
            while (elem) {
                atf_ci::FBuilddir &row        = *elem;
                atf_ci::FBuilddir* next       = row.ind_builddir_next;
                u32 index          = algo::Smallstr50_Hash(0, row.builddir) & (new_nbuckets-1);
                row.ind_builddir_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_builddir_buckets_elems, old_size);
        _db.ind_builddir_buckets_elems = new_buckets;
        _db.ind_builddir_buckets_n = new_nbuckets;
    }
}

// --- atf_ci.FDb.gitfile.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FGitfile& atf_ci::gitfile_Alloc() {
    atf_ci::FGitfile* row = gitfile_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.gitfile  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.gitfile.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FGitfile* atf_ci::gitfile_AllocMaybe() {
    atf_ci::FGitfile *row = (atf_ci::FGitfile*)gitfile_AllocMem();
    if (row) {
        new (row) atf_ci::FGitfile; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.gitfile.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FGitfile* atf_ci::gitfile_InsertMaybe(const dev::Gitfile &value) {
    atf_ci::FGitfile *row = &gitfile_Alloc(); // if out of memory, process dies. if input error, return NULL.
    gitfile_CopyIn(*row,const_cast<dev::Gitfile&>(value));
    bool ok = gitfile_XrefMaybe(*row); // this may return false
    if (!ok) {
        gitfile_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.gitfile.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::gitfile_AllocMem() {
    u64 new_nelems     = _db.gitfile_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FGitfile*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.gitfile_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FGitfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FGitfile) * (u64(1)<<bsr));
            _db.gitfile_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.gitfile_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.gitfile.RemoveAll
// Remove all elements from Lary
void atf_ci::gitfile_RemoveAll() {
    for (u64 n = _db.gitfile_n; n>0; ) {
        n--;
        gitfile_qFind(u64(n)).~FGitfile(); // destroy last element
        _db.gitfile_n = i32(n);
    }
}

// --- atf_ci.FDb.gitfile.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::gitfile_RemoveLast() {
    u64 n = _db.gitfile_n;
    if (n > 0) {
        n -= 1;
        gitfile_qFind(u64(n)).~FGitfile();
        _db.gitfile_n = i32(n);
    }
}

// --- atf_ci.FDb.gitfile.InputMaybe
static bool atf_ci::gitfile_InputMaybe(dev::Gitfile &elem) {
    bool retval = true;
    retval = gitfile_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.gitfile.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::gitfile_XrefMaybe(atf_ci::FGitfile &row) {
    bool retval = true;
    (void)row;
    // insert gitfile into index ind_gitfile
    if (true) { // user-defined insert condition
        bool success = ind_gitfile_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FDb.ind_gitfile"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_gitfile.Find
// Find row by key. Return NULL if not found.
atf_ci::FGitfile* atf_ci::ind_gitfile_Find(const algo::strptr& key) {
    u32 index = algo::Smallstr200_Hash(0, key) & (_db.ind_gitfile_buckets_n - 1);
    atf_ci::FGitfile* *e = &_db.ind_gitfile_buckets_elems[index];
    atf_ci::FGitfile* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).gitfile == key;
        if (done) break;
        e         = &ret->ind_gitfile_next;
    } while (true);
    return ret;
}

// --- atf_ci.FDb.ind_gitfile.FindX
// Look up row by key and return reference. Throw exception if not found
atf_ci::FGitfile& atf_ci::ind_gitfile_FindX(const algo::strptr& key) {
    atf_ci::FGitfile* ret = ind_gitfile_Find(key);
    vrfy(ret, tempstr() << "atf_ci.key_error  table:ind_gitfile  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- atf_ci.FDb.ind_gitfile.GetOrCreate
// Find row by key. If not found, create and x-reference a new row with with this key.
atf_ci::FGitfile& atf_ci::ind_gitfile_GetOrCreate(const algo::strptr& key) {
    atf_ci::FGitfile* ret = ind_gitfile_Find(key);
    if (!ret) { //  if memory alloc fails, process dies; if insert fails, function returns NULL.
        ret         = &gitfile_Alloc();
        (*ret).gitfile = key;
        bool good = gitfile_XrefMaybe(*ret);
        if (!good) {
            gitfile_RemoveLast(); // delete offending row, any existing xrefs are cleared
            ret = NULL;
        }
    }
    vrfy(ret, tempstr() << "atf_ci.create_error  table:ind_gitfile  key:'"<<key<<"'  comment:'bad xref'");
    return *ret;
}

// --- atf_ci.FDb.ind_gitfile.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool atf_ci::ind_gitfile_InsertMaybe(atf_ci::FGitfile& row) {
    ind_gitfile_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_gitfile_next == (atf_ci::FGitfile*)-1)) {// check if in hash already
        u32 index = algo::Smallstr200_Hash(0, row.gitfile) & (_db.ind_gitfile_buckets_n - 1);
        atf_ci::FGitfile* *prev = &_db.ind_gitfile_buckets_elems[index];
        do {
            atf_ci::FGitfile* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).gitfile == row.gitfile) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_gitfile_next;
        } while (true);
        if (retval) {
            row.ind_gitfile_next = *prev;
            _db.ind_gitfile_n++;
            *prev = &row;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_gitfile.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void atf_ci::ind_gitfile_Remove(atf_ci::FGitfile& row) {
    if (LIKELY(row.ind_gitfile_next != (atf_ci::FGitfile*)-1)) {// check if in hash already
        u32 index = algo::Smallstr200_Hash(0, row.gitfile) & (_db.ind_gitfile_buckets_n - 1);
        atf_ci::FGitfile* *prev = &_db.ind_gitfile_buckets_elems[index]; // addr of pointer to current element
        while (atf_ci::FGitfile *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_gitfile_next; // unlink (singly linked list)
                _db.ind_gitfile_n--;
                row.ind_gitfile_next = (atf_ci::FGitfile*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_gitfile_next;
        }
    }
}

// --- atf_ci.FDb.ind_gitfile.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void atf_ci::ind_gitfile_Reserve(int n) {
    u32 old_nbuckets = _db.ind_gitfile_buckets_n;
    u32 new_nelems   = _db.ind_gitfile_n + n;
    // # of elements has to be roughly equal to the number of buckets
    if (new_nelems > old_nbuckets) {
        int new_nbuckets = i32_Max(algo::BumpToPow2(new_nelems), u32(4));
        u32 old_size = old_nbuckets * sizeof(atf_ci::FGitfile*);
        u32 new_size = new_nbuckets * sizeof(atf_ci::FGitfile*);
        // allocate new array. we don't use Realloc since copying is not needed and factor of 2 probably
        // means new memory will have to be allocated anyway
        atf_ci::FGitfile* *new_buckets = (atf_ci::FGitfile**)algo_lib::malloc_AllocMem(new_size);
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("atf_ci.out_of_memory  field:atf_ci.FDb.ind_gitfile");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_gitfile_buckets_n; i++) {
            atf_ci::FGitfile* elem = _db.ind_gitfile_buckets_elems[i];
            while (elem) {
                atf_ci::FGitfile &row        = *elem;
                atf_ci::FGitfile* next       = row.ind_gitfile_next;
                u32 index          = algo::Smallstr200_Hash(0, row.gitfile) & (new_nbuckets-1);
                row.ind_gitfile_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_gitfile_buckets_elems, old_size);
        _db.ind_gitfile_buckets_elems = new_buckets;
        _db.ind_gitfile_buckets_n = new_nbuckets;
    }
}

// --- atf_ci.FDb.noindent.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FNoindent& atf_ci::noindent_Alloc() {
    atf_ci::FNoindent* row = noindent_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.noindent  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.noindent.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FNoindent* atf_ci::noindent_AllocMaybe() {
    atf_ci::FNoindent *row = (atf_ci::FNoindent*)noindent_AllocMem();
    if (row) {
        new (row) atf_ci::FNoindent; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.noindent.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FNoindent* atf_ci::noindent_InsertMaybe(const dev::Noindent &value) {
    atf_ci::FNoindent *row = &noindent_Alloc(); // if out of memory, process dies. if input error, return NULL.
    noindent_CopyIn(*row,const_cast<dev::Noindent&>(value));
    bool ok = noindent_XrefMaybe(*row); // this may return false
    if (!ok) {
        noindent_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.noindent.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::noindent_AllocMem() {
    u64 new_nelems     = _db.noindent_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FNoindent*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.noindent_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FNoindent*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FNoindent) * (u64(1)<<bsr));
            _db.noindent_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.noindent_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.noindent.RemoveAll
// Remove all elements from Lary
void atf_ci::noindent_RemoveAll() {
    for (u64 n = _db.noindent_n; n>0; ) {
        n--;
        noindent_qFind(u64(n)).~FNoindent(); // destroy last element
        _db.noindent_n = i32(n);
    }
}

// --- atf_ci.FDb.noindent.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::noindent_RemoveLast() {
    u64 n = _db.noindent_n;
    if (n > 0) {
        n -= 1;
        noindent_qFind(u64(n)).~FNoindent();
        _db.noindent_n = i32(n);
    }
}

// --- atf_ci.FDb.noindent.InputMaybe
static bool atf_ci::noindent_InputMaybe(dev::Noindent &elem) {
    bool retval = true;
    retval = noindent_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.noindent.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::noindent_XrefMaybe(atf_ci::FNoindent &row) {
    bool retval = true;
    (void)row;
    atf_ci::FGitfile* p_gitfile = atf_ci::ind_gitfile_Find(row.gitfile);
    if (UNLIKELY(!p_gitfile)) {
        algo_lib::ResetErrtext() << "atf_ci.bad_xref  index:atf_ci.FDb.ind_gitfile" << Keyval("key", row.gitfile);
        return false;
    }
    // insert noindent into index c_noindent
    if (true) { // user-defined insert condition
        bool success = c_noindent_InsertMaybe(*p_gitfile, row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FGitfile.c_noindent"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.targsrc.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FTargsrc& atf_ci::targsrc_Alloc() {
    atf_ci::FTargsrc* row = targsrc_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.targsrc  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.targsrc.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FTargsrc* atf_ci::targsrc_AllocMaybe() {
    atf_ci::FTargsrc *row = (atf_ci::FTargsrc*)targsrc_AllocMem();
    if (row) {
        new (row) atf_ci::FTargsrc; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.targsrc.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FTargsrc* atf_ci::targsrc_InsertMaybe(const dev::Targsrc &value) {
    atf_ci::FTargsrc *row = &targsrc_Alloc(); // if out of memory, process dies. if input error, return NULL.
    targsrc_CopyIn(*row,const_cast<dev::Targsrc&>(value));
    bool ok = targsrc_XrefMaybe(*row); // this may return false
    if (!ok) {
        targsrc_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.targsrc.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::targsrc_AllocMem() {
    u64 new_nelems     = _db.targsrc_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FTargsrc*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.targsrc_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FTargsrc*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FTargsrc) * (u64(1)<<bsr));
            _db.targsrc_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.targsrc_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.targsrc.RemoveAll
// Remove all elements from Lary
void atf_ci::targsrc_RemoveAll() {
    for (u64 n = _db.targsrc_n; n>0; ) {
        n--;
        targsrc_qFind(u64(n)).~FTargsrc(); // destroy last element
        _db.targsrc_n = i32(n);
    }
}

// --- atf_ci.FDb.targsrc.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::targsrc_RemoveLast() {
    u64 n = _db.targsrc_n;
    if (n > 0) {
        n -= 1;
        targsrc_qFind(u64(n)).~FTargsrc();
        _db.targsrc_n = i32(n);
    }
}

// --- atf_ci.FDb.targsrc.InputMaybe
static bool atf_ci::targsrc_InputMaybe(dev::Targsrc &elem) {
    bool retval = true;
    retval = targsrc_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.targsrc.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::targsrc_XrefMaybe(atf_ci::FTargsrc &row) {
    bool retval = true;
    (void)row;
    atf_ci::FGitfile* p_src = atf_ci::ind_gitfile_Find(src_Get(row));
    if (UNLIKELY(!p_src)) {
        algo_lib::ResetErrtext() << "atf_ci.bad_xref  index:atf_ci.FDb.ind_gitfile" << Keyval("key", src_Get(row));
        return false;
    }
    // insert targsrc into index c_targsrc
    if (true) { // user-defined insert condition
        bool success = c_targsrc_InsertMaybe(*p_src, row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "atf_ci.duplicate_key  xref:atf_ci.FGitfile.c_targsrc"; // check for duplicate key
            return false;
        }
    }
    return retval;
}

// --- atf_ci.FDb.msgfile.Alloc
// Allocate memory for new default row.
// If out of memory, process is killed.
atf_ci::FMsgfile& atf_ci::msgfile_Alloc() {
    atf_ci::FMsgfile* row = msgfile_AllocMaybe();
    if (UNLIKELY(row == NULL)) {
        FatalErrorExit("atf_ci.out_of_mem  field:atf_ci.FDb.msgfile  comment:'Alloc failed'");
    }
    return *row;
}

// --- atf_ci.FDb.msgfile.AllocMaybe
// Allocate memory for new element. If out of memory, return NULL.
atf_ci::FMsgfile* atf_ci::msgfile_AllocMaybe() {
    atf_ci::FMsgfile *row = (atf_ci::FMsgfile*)msgfile_AllocMem();
    if (row) {
        new (row) atf_ci::FMsgfile; // call constructor
    }
    return row;
}

// --- atf_ci.FDb.msgfile.InsertMaybe
// Create new row from struct.
// Return pointer to new element, or NULL if insertion failed (due to out-of-memory, duplicate key, etc)
atf_ci::FMsgfile* atf_ci::msgfile_InsertMaybe(const dev::Msgfile &value) {
    atf_ci::FMsgfile *row = &msgfile_Alloc(); // if out of memory, process dies. if input error, return NULL.
    msgfile_CopyIn(*row,const_cast<dev::Msgfile&>(value));
    bool ok = msgfile_XrefMaybe(*row); // this may return false
    if (!ok) {
        msgfile_RemoveLast(); // delete offending row, any existing xrefs are cleared
        row = NULL; // forget this ever happened
    }
    return row;
}

// --- atf_ci.FDb.msgfile.AllocMem
// Allocate space for one element. If no memory available, return NULL.
void* atf_ci::msgfile_AllocMem() {
    u64 new_nelems     = _db.msgfile_n+1;
    // compute level and index on level
    u64 bsr   = algo::u64_BitScanReverse(new_nelems);
    u64 base  = u64(1)<<bsr;
    u64 index = new_nelems-base;
    void *ret = NULL;
    // if level doesn't exist yet, create it
    atf_ci::FMsgfile*  lev   = NULL;
    if (bsr < 32) {
        lev = _db.msgfile_lary[bsr];
        if (!lev) {
            lev=(atf_ci::FMsgfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FMsgfile) * (u64(1)<<bsr));
            _db.msgfile_lary[bsr] = lev;
        }
    }
    // allocate element from this level
    if (lev) {
        _db.msgfile_n = i32(new_nelems);
        ret = lev + index;
    }
    return ret;
}

// --- atf_ci.FDb.msgfile.RemoveAll
// Remove all elements from Lary
void atf_ci::msgfile_RemoveAll() {
    for (u64 n = _db.msgfile_n; n>0; ) {
        n--;
        msgfile_qFind(u64(n)).~FMsgfile(); // destroy last element
        _db.msgfile_n = i32(n);
    }
}

// --- atf_ci.FDb.msgfile.RemoveLast
// Delete last element of array. Do nothing if array is empty.
void atf_ci::msgfile_RemoveLast() {
    u64 n = _db.msgfile_n;
    if (n > 0) {
        n -= 1;
        msgfile_qFind(u64(n)).~FMsgfile();
        _db.msgfile_n = i32(n);
    }
}

// --- atf_ci.FDb.msgfile.InputMaybe
static bool atf_ci::msgfile_InputMaybe(dev::Msgfile &elem) {
    bool retval = true;
    retval = msgfile_InsertMaybe(elem) != nullptr;
    return retval;
}

// --- atf_ci.FDb.msgfile.XrefMaybe
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
bool atf_ci::msgfile_XrefMaybe(atf_ci::FMsgfile &row) {
    bool retval = true;
    (void)row;
    return retval;
}

// --- atf_ci.FDb.ind_citest.Find
// Find row by key. Return NULL if not found.
atf_ci::FCitest* atf_ci::ind_citest_Find(const algo::strptr& key) {
    u32 index = algo::Smallstr50_Hash(0, key) & (_db.ind_citest_buckets_n - 1);
    atf_ci::FCitest* *e = &_db.ind_citest_buckets_elems[index];
    atf_ci::FCitest* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).citest == key;
        if (done) break;
        e         = &ret->ind_citest_next;
    } while (true);
    return ret;
}

// --- atf_ci.FDb.ind_citest.FindX
// Look up row by key and return reference. Throw exception if not found
atf_ci::FCitest& atf_ci::ind_citest_FindX(const algo::strptr& key) {
    atf_ci::FCitest* ret = ind_citest_Find(key);
    vrfy(ret, tempstr() << "atf_ci.key_error  table:ind_citest  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- atf_ci.FDb.ind_citest.GetOrCreate
// Find row by key. If not found, create and x-reference a new row with with this key.
atf_ci::FCitest& atf_ci::ind_citest_GetOrCreate(const algo::strptr& key) {
    atf_ci::FCitest* ret = ind_citest_Find(key);
    if (!ret) { //  if memory alloc fails, process dies; if insert fails, function returns NULL.
        ret         = &citest_Alloc();
        (*ret).citest = key;
        bool good = citest_XrefMaybe(*ret);
        if (!good) {
            citest_RemoveLast(); // delete offending row, any existing xrefs are cleared
            ret = NULL;
        }
    }
    vrfy(ret, tempstr() << "atf_ci.create_error  table:ind_citest  key:'"<<key<<"'  comment:'bad xref'");
    return *ret;
}

// --- atf_ci.FDb.ind_citest.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool atf_ci::ind_citest_InsertMaybe(atf_ci::FCitest& row) {
    ind_citest_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_citest_next == (atf_ci::FCitest*)-1)) {// check if in hash already
        u32 index = algo::Smallstr50_Hash(0, row.citest) & (_db.ind_citest_buckets_n - 1);
        atf_ci::FCitest* *prev = &_db.ind_citest_buckets_elems[index];
        do {
            atf_ci::FCitest* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).citest == row.citest) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_citest_next;
        } while (true);
        if (retval) {
            row.ind_citest_next = *prev;
            _db.ind_citest_n++;
            *prev = &row;
        }
    }
    return retval;
}

// --- atf_ci.FDb.ind_citest.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void atf_ci::ind_citest_Remove(atf_ci::FCitest& row) {
    if (LIKELY(row.ind_citest_next != (atf_ci::FCitest*)-1)) {// check if in hash already
        u32 index = algo::Smallstr50_Hash(0, row.citest) & (_db.ind_citest_buckets_n - 1);
        atf_ci::FCitest* *prev = &_db.ind_citest_buckets_elems[index]; // addr of pointer to current element
        while (atf_ci::FCitest *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_citest_next; // unlink (singly linked list)
                _db.ind_citest_n--;
                row.ind_citest_next = (atf_ci::FCitest*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_citest_next;
        }
    }
}

// --- atf_ci.FDb.ind_citest.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void atf_ci::ind_citest_Reserve(int n) {
    u32 old_nbuckets = _db.ind_citest_buckets_n;
    u32 new_nelems   = _db.ind_citest_n + n;
    // # of elements has to be roughly equal to the number of buckets
    if (new_nelems > old_nbuckets) {
        int new_nbuckets = i32_Max(algo::BumpToPow2(new_nelems), u32(4));
        u32 old_size = old_nbuckets * sizeof(atf_ci::FCitest*);
        u32 new_size = new_nbuckets * sizeof(atf_ci::FCitest*);
        // allocate new array. we don't use Realloc since copying is not needed and factor of 2 probably
        // means new memory will have to be allocated anyway
        atf_ci::FCitest* *new_buckets = (atf_ci::FCitest**)algo_lib::malloc_AllocMem(new_size);
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("atf_ci.out_of_memory  field:atf_ci.FDb.ind_citest");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_citest_buckets_n; i++) {
            atf_ci::FCitest* elem = _db.ind_citest_buckets_elems[i];
            while (elem) {
                atf_ci::FCitest &row        = *elem;
                atf_ci::FCitest* next       = row.ind_citest_next;
                u32 index          = algo::Smallstr50_Hash(0, row.citest) & (new_nbuckets-1);
                row.ind_citest_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_citest_buckets_elems, old_size);
        _db.ind_citest_buckets_elems = new_buckets;
        _db.ind_citest_buckets_n = new_nbuckets;
    }
}

// --- atf_ci.FDb.trace.RowidFind
// find trace by row id (used to implement reflection)
static algo::ImrowPtr atf_ci::trace_RowidFind(int t) {
    return algo::ImrowPtr(t==0 ? u64(&_db.trace) : u64(0));
}

// --- atf_ci.FDb.trace.N
// Function return 1
inline static i32 atf_ci::trace_N() {
    return 1;
}

// --- atf_ci.FDb..Init
// Set all fields to initial values.
void atf_ci::FDb_Init() {
    // initialize LAry citest (atf_ci.FDb.citest)
    _db.citest_n = 0;
    memset(_db.citest_lary, 0, sizeof(_db.citest_lary)); // zero out all level pointers
    atf_ci::FCitest* citest_first = (atf_ci::FCitest*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FCitest) * (u64(1)<<4));
    if (!citest_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.citest_lary[i]  = citest_first;
        citest_first    += 1ULL<<i;
    }
    // initialize LAry ssimfile (atf_ci.FDb.ssimfile)
    _db.ssimfile_n = 0;
    memset(_db.ssimfile_lary, 0, sizeof(_db.ssimfile_lary)); // zero out all level pointers
    atf_ci::FSsimfile* ssimfile_first = (atf_ci::FSsimfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FSsimfile) * (u64(1)<<4));
    if (!ssimfile_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.ssimfile_lary[i]  = ssimfile_first;
        ssimfile_first    += 1ULL<<i;
    }
    // initialize hash table for atf_ci::FSsimfile;
    _db.ind_ssimfile_n             	= 0; // (atf_ci.FDb.ind_ssimfile)
    _db.ind_ssimfile_buckets_n     	= 4; // (atf_ci.FDb.ind_ssimfile)
    _db.ind_ssimfile_buckets_elems 	= (atf_ci::FSsimfile**)algo_lib::malloc_AllocMem(sizeof(atf_ci::FSsimfile*)*_db.ind_ssimfile_buckets_n); // initial buckets (atf_ci.FDb.ind_ssimfile)
    if (!_db.ind_ssimfile_buckets_elems) {
        FatalErrorExit("out of memory"); // (atf_ci.FDb.ind_ssimfile)
    }
    memset(_db.ind_ssimfile_buckets_elems, 0, sizeof(atf_ci::FSsimfile*)*_db.ind_ssimfile_buckets_n); // (atf_ci.FDb.ind_ssimfile)
    _db.c_citest = NULL;
    // initialize LAry scriptfile (atf_ci.FDb.scriptfile)
    _db.scriptfile_n = 0;
    memset(_db.scriptfile_lary, 0, sizeof(_db.scriptfile_lary)); // zero out all level pointers
    atf_ci::FScriptfile* scriptfile_first = (atf_ci::FScriptfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FScriptfile) * (u64(1)<<4));
    if (!scriptfile_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.scriptfile_lary[i]  = scriptfile_first;
        scriptfile_first    += 1ULL<<i;
    }
    // initialize hash table for atf_ci::FScriptfile;
    _db.ind_scriptfile_n             	= 0; // (atf_ci.FDb.ind_scriptfile)
    _db.ind_scriptfile_buckets_n     	= 4; // (atf_ci.FDb.ind_scriptfile)
    _db.ind_scriptfile_buckets_elems 	= (atf_ci::FScriptfile**)algo_lib::malloc_AllocMem(sizeof(atf_ci::FScriptfile*)*_db.ind_scriptfile_buckets_n); // initial buckets (atf_ci.FDb.ind_scriptfile)
    if (!_db.ind_scriptfile_buckets_elems) {
        FatalErrorExit("out of memory"); // (atf_ci.FDb.ind_scriptfile)
    }
    memset(_db.ind_scriptfile_buckets_elems, 0, sizeof(atf_ci::FScriptfile*)*_db.ind_scriptfile_buckets_n); // (atf_ci.FDb.ind_scriptfile)
    // initialize LAry ns (atf_ci.FDb.ns)
    _db.ns_n = 0;
    memset(_db.ns_lary, 0, sizeof(_db.ns_lary)); // zero out all level pointers
    atf_ci::FNs* ns_first = (atf_ci::FNs*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FNs) * (u64(1)<<4));
    if (!ns_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.ns_lary[i]  = ns_first;
        ns_first    += 1ULL<<i;
    }
    // initialize hash table for atf_ci::FNs;
    _db.ind_ns_n             	= 0; // (atf_ci.FDb.ind_ns)
    _db.ind_ns_buckets_n     	= 4; // (atf_ci.FDb.ind_ns)
    _db.ind_ns_buckets_elems 	= (atf_ci::FNs**)algo_lib::malloc_AllocMem(sizeof(atf_ci::FNs*)*_db.ind_ns_buckets_n); // initial buckets (atf_ci.FDb.ind_ns)
    if (!_db.ind_ns_buckets_elems) {
        FatalErrorExit("out of memory"); // (atf_ci.FDb.ind_ns)
    }
    memset(_db.ind_ns_buckets_elems, 0, sizeof(atf_ci::FNs*)*_db.ind_ns_buckets_n); // (atf_ci.FDb.ind_ns)
    // initialize LAry readme (atf_ci.FDb.readme)
    _db.readme_n = 0;
    memset(_db.readme_lary, 0, sizeof(_db.readme_lary)); // zero out all level pointers
    atf_ci::FReadme* readme_first = (atf_ci::FReadme*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FReadme) * (u64(1)<<4));
    if (!readme_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.readme_lary[i]  = readme_first;
        readme_first    += 1ULL<<i;
    }
    // initialize LAry builddir (atf_ci.FDb.builddir)
    _db.builddir_n = 0;
    memset(_db.builddir_lary, 0, sizeof(_db.builddir_lary)); // zero out all level pointers
    atf_ci::FBuilddir* builddir_first = (atf_ci::FBuilddir*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FBuilddir) * (u64(1)<<4));
    if (!builddir_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.builddir_lary[i]  = builddir_first;
        builddir_first    += 1ULL<<i;
    }
    // initialize LAry cfg (atf_ci.FDb.cfg)
    _db.cfg_n = 0;
    memset(_db.cfg_lary, 0, sizeof(_db.cfg_lary)); // zero out all level pointers
    atf_ci::FCfg* cfg_first = (atf_ci::FCfg*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FCfg) * (u64(1)<<4));
    if (!cfg_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.cfg_lary[i]  = cfg_first;
        cfg_first    += 1ULL<<i;
    }
    // initialize hash table for atf_ci::FBuilddir;
    _db.ind_builddir_n             	= 0; // (atf_ci.FDb.ind_builddir)
    _db.ind_builddir_buckets_n     	= 4; // (atf_ci.FDb.ind_builddir)
    _db.ind_builddir_buckets_elems 	= (atf_ci::FBuilddir**)algo_lib::malloc_AllocMem(sizeof(atf_ci::FBuilddir*)*_db.ind_builddir_buckets_n); // initial buckets (atf_ci.FDb.ind_builddir)
    if (!_db.ind_builddir_buckets_elems) {
        FatalErrorExit("out of memory"); // (atf_ci.FDb.ind_builddir)
    }
    memset(_db.ind_builddir_buckets_elems, 0, sizeof(atf_ci::FBuilddir*)*_db.ind_builddir_buckets_n); // (atf_ci.FDb.ind_builddir)
    // initialize LAry gitfile (atf_ci.FDb.gitfile)
    _db.gitfile_n = 0;
    memset(_db.gitfile_lary, 0, sizeof(_db.gitfile_lary)); // zero out all level pointers
    atf_ci::FGitfile* gitfile_first = (atf_ci::FGitfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FGitfile) * (u64(1)<<4));
    if (!gitfile_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.gitfile_lary[i]  = gitfile_first;
        gitfile_first    += 1ULL<<i;
    }
    // initialize hash table for atf_ci::FGitfile;
    _db.ind_gitfile_n             	= 0; // (atf_ci.FDb.ind_gitfile)
    _db.ind_gitfile_buckets_n     	= 4; // (atf_ci.FDb.ind_gitfile)
    _db.ind_gitfile_buckets_elems 	= (atf_ci::FGitfile**)algo_lib::malloc_AllocMem(sizeof(atf_ci::FGitfile*)*_db.ind_gitfile_buckets_n); // initial buckets (atf_ci.FDb.ind_gitfile)
    if (!_db.ind_gitfile_buckets_elems) {
        FatalErrorExit("out of memory"); // (atf_ci.FDb.ind_gitfile)
    }
    memset(_db.ind_gitfile_buckets_elems, 0, sizeof(atf_ci::FGitfile*)*_db.ind_gitfile_buckets_n); // (atf_ci.FDb.ind_gitfile)
    // initialize LAry noindent (atf_ci.FDb.noindent)
    _db.noindent_n = 0;
    memset(_db.noindent_lary, 0, sizeof(_db.noindent_lary)); // zero out all level pointers
    atf_ci::FNoindent* noindent_first = (atf_ci::FNoindent*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FNoindent) * (u64(1)<<4));
    if (!noindent_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.noindent_lary[i]  = noindent_first;
        noindent_first    += 1ULL<<i;
    }
    // initialize LAry targsrc (atf_ci.FDb.targsrc)
    _db.targsrc_n = 0;
    memset(_db.targsrc_lary, 0, sizeof(_db.targsrc_lary)); // zero out all level pointers
    atf_ci::FTargsrc* targsrc_first = (atf_ci::FTargsrc*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FTargsrc) * (u64(1)<<4));
    if (!targsrc_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.targsrc_lary[i]  = targsrc_first;
        targsrc_first    += 1ULL<<i;
    }
    // initialize LAry msgfile (atf_ci.FDb.msgfile)
    _db.msgfile_n = 0;
    memset(_db.msgfile_lary, 0, sizeof(_db.msgfile_lary)); // zero out all level pointers
    atf_ci::FMsgfile* msgfile_first = (atf_ci::FMsgfile*)algo_lib::malloc_AllocMem(sizeof(atf_ci::FMsgfile) * (u64(1)<<4));
    if (!msgfile_first) {
        FatalErrorExit("out of memory");
    }
    for (int i = 0; i < 4; i++) {
        _db.msgfile_lary[i]  = msgfile_first;
        msgfile_first    += 1ULL<<i;
    }
    _db.preped_env = bool(false);
    // initialize hash table for atf_ci::FCitest;
    _db.ind_citest_n             	= 0; // (atf_ci.FDb.ind_citest)
    _db.ind_citest_buckets_n     	= 4; // (atf_ci.FDb.ind_citest)
    _db.ind_citest_buckets_elems 	= (atf_ci::FCitest**)algo_lib::malloc_AllocMem(sizeof(atf_ci::FCitest*)*_db.ind_citest_buckets_n); // initial buckets (atf_ci.FDb.ind_citest)
    if (!_db.ind_citest_buckets_elems) {
        FatalErrorExit("out of memory"); // (atf_ci.FDb.ind_citest)
    }
    memset(_db.ind_citest_buckets_elems, 0, sizeof(atf_ci::FCitest*)*_db.ind_citest_buckets_n); // (atf_ci.FDb.ind_citest)

    atf_ci::InitReflection();
    citest_LoadStatic();
}

// --- atf_ci.FDb..Uninit
void atf_ci::FDb_Uninit() {
    atf_ci::FDb &row = _db; (void)row;

    // atf_ci.FDb.ind_citest.Uninit (Thash)  //
    // skip destruction of ind_citest in global scope

    // atf_ci.FDb.msgfile.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.targsrc.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.noindent.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.ind_gitfile.Uninit (Thash)  //
    // skip destruction of ind_gitfile in global scope

    // atf_ci.FDb.gitfile.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.ind_builddir.Uninit (Thash)  //
    // skip destruction of ind_builddir in global scope

    // atf_ci.FDb.cfg.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.builddir.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.readme.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.ind_ns.Uninit (Thash)  //
    // skip destruction of ind_ns in global scope

    // atf_ci.FDb.ns.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.ind_scriptfile.Uninit (Thash)  //
    // skip destruction of ind_scriptfile in global scope

    // atf_ci.FDb.scriptfile.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.ind_ssimfile.Uninit (Thash)  //
    // skip destruction of ind_ssimfile in global scope

    // atf_ci.FDb.ssimfile.Uninit (Lary)  //
    // skip destruction in global scope

    // atf_ci.FDb.citest.Uninit (Lary)  //
    // skip destruction in global scope
}

// --- atf_ci.FGitfile.base.CopyOut
// Copy fields out of row
void atf_ci::gitfile_CopyOut(atf_ci::FGitfile &row, dev::Gitfile &out) {
    out.gitfile = row.gitfile;
}

// --- atf_ci.FGitfile.base.CopyIn
// Copy fields in to row
void atf_ci::gitfile_CopyIn(atf_ci::FGitfile &row, dev::Gitfile &in) {
    row.gitfile = in.gitfile;
}

// --- atf_ci.FGitfile.ext.Get
algo::Smallstr50 atf_ci::ext_Get(atf_ci::FGitfile& gitfile) {
    algo::Smallstr50 ret(algo::Pathcomp(gitfile.gitfile, "/RR.LR.RR"));
    return ret;
}

// --- atf_ci.FGitfile..Uninit
void atf_ci::FGitfile_Uninit(atf_ci::FGitfile& gitfile) {
    atf_ci::FGitfile &row = gitfile; (void)row;
    ind_gitfile_Remove(row); // remove gitfile from index ind_gitfile
}

// --- atf_ci.FMsgfile.base.CopyOut
// Copy fields out of row
void atf_ci::msgfile_CopyOut(atf_ci::FMsgfile &row, dev::Msgfile &out) {
    out.gitfile = row.gitfile;
    out.strict = row.strict;
    out.comment = row.comment;
}

// --- atf_ci.FMsgfile.base.CopyIn
// Copy fields in to row
void atf_ci::msgfile_CopyIn(atf_ci::FMsgfile &row, dev::Msgfile &in) {
    row.gitfile = in.gitfile;
    row.strict = in.strict;
    row.comment = in.comment;
}

// --- atf_ci.FNoindent.base.CopyOut
// Copy fields out of row
void atf_ci::noindent_CopyOut(atf_ci::FNoindent &row, dev::Noindent &out) {
    out.gitfile = row.gitfile;
    out.comment = row.comment;
}

// --- atf_ci.FNoindent.base.CopyIn
// Copy fields in to row
void atf_ci::noindent_CopyIn(atf_ci::FNoindent &row, dev::Noindent &in) {
    row.gitfile = in.gitfile;
    row.comment = in.comment;
}

// --- atf_ci.FNoindent..Uninit
void atf_ci::FNoindent_Uninit(atf_ci::FNoindent& noindent) {
    atf_ci::FNoindent &row = noindent; (void)row;
    atf_ci::FGitfile* p_gitfile = atf_ci::ind_gitfile_Find(row.gitfile);
    if (p_gitfile)  {
        c_noindent_Remove(*p_gitfile, row);// remove noindent from index c_noindent
    }
}

// --- atf_ci.FNs.base.CopyOut
// Copy fields out of row
void atf_ci::ns_CopyOut(atf_ci::FNs &row, dmmeta::Ns &out) {
    out.ns = row.ns;
    out.nstype = row.nstype;
    out.comment = row.comment;
}

// --- atf_ci.FNs.base.CopyIn
// Copy fields in to row
void atf_ci::ns_CopyIn(atf_ci::FNs &row, dmmeta::Ns &in) {
    row.ns = in.ns;
    row.nstype = in.nstype;
    row.comment = in.comment;
}

// --- atf_ci.FNs..Uninit
void atf_ci::FNs_Uninit(atf_ci::FNs& ns) {
    atf_ci::FNs &row = ns; (void)row;
    ind_ns_Remove(row); // remove ns from index ind_ns
}

// --- atf_ci.FReadme.base.CopyOut
// Copy fields out of row
void atf_ci::readme_CopyOut(atf_ci::FReadme &row, dev::Readme &out) {
    out.gitfile = row.gitfile;
    out.inl = row.inl;
    out.comment = row.comment;
}

// --- atf_ci.FReadme.base.CopyIn
// Copy fields in to row
void atf_ci::readme_CopyIn(atf_ci::FReadme &row, dev::Readme &in) {
    row.gitfile = in.gitfile;
    row.inl = in.inl;
    row.comment = in.comment;
}

// --- atf_ci.FScriptfile.base.CopyOut
// Copy fields out of row
void atf_ci::scriptfile_CopyOut(atf_ci::FScriptfile &row, dev::Scriptfile &out) {
    out.gitfile = row.gitfile;
    out.args = row.args;
    out.comment = row.comment;
}

// --- atf_ci.FScriptfile.base.CopyIn
// Copy fields in to row
void atf_ci::scriptfile_CopyIn(atf_ci::FScriptfile &row, dev::Scriptfile &in) {
    row.gitfile = in.gitfile;
    row.args = in.args;
    row.comment = in.comment;
}

// --- atf_ci.FScriptfile..Uninit
void atf_ci::FScriptfile_Uninit(atf_ci::FScriptfile& scriptfile) {
    atf_ci::FScriptfile &row = scriptfile; (void)row;
    ind_scriptfile_Remove(row); // remove scriptfile from index ind_scriptfile
    atf_ci::FGitfile* p_gitfile = atf_ci::ind_gitfile_Find(row.gitfile);
    if (p_gitfile)  {
        c_scriptfile_Remove(*p_gitfile, row);// remove scriptfile from index c_scriptfile
    }
}

// --- atf_ci.FSsimfile.base.CopyOut
// Copy fields out of row
void atf_ci::ssimfile_CopyOut(atf_ci::FSsimfile &row, dmmeta::Ssimfile &out) {
    out.ssimfile = row.ssimfile;
    out.ctype = row.ctype;
}

// --- atf_ci.FSsimfile.base.CopyIn
// Copy fields in to row
void atf_ci::ssimfile_CopyIn(atf_ci::FSsimfile &row, dmmeta::Ssimfile &in) {
    row.ssimfile = in.ssimfile;
    row.ctype = in.ctype;
}

// --- atf_ci.FSsimfile.ssimns.Get
algo::Smallstr16 atf_ci::ssimns_Get(atf_ci::FSsimfile& ssimfile) {
    algo::Smallstr16 ret(algo::Pathcomp(ssimfile.ssimfile, ".LL"));
    return ret;
}

// --- atf_ci.FSsimfile.ns.Get
algo::Smallstr16 atf_ci::ns_Get(atf_ci::FSsimfile& ssimfile) {
    algo::Smallstr16 ret(algo::Pathcomp(ssimfile.ssimfile, ".LL"));
    return ret;
}

// --- atf_ci.FSsimfile.name.Get
algo::Smallstr50 atf_ci::name_Get(atf_ci::FSsimfile& ssimfile) {
    algo::Smallstr50 ret(algo::Pathcomp(ssimfile.ssimfile, ".RR"));
    return ret;
}

// --- atf_ci.FSsimfile..Uninit
void atf_ci::FSsimfile_Uninit(atf_ci::FSsimfile& ssimfile) {
    atf_ci::FSsimfile &row = ssimfile; (void)row;
    ind_ssimfile_Remove(row); // remove ssimfile from index ind_ssimfile
}

// --- atf_ci.FTargsrc.base.CopyOut
// Copy fields out of row
void atf_ci::targsrc_CopyOut(atf_ci::FTargsrc &row, dev::Targsrc &out) {
    out.targsrc = row.targsrc;
    out.comment = row.comment;
}

// --- atf_ci.FTargsrc.base.CopyIn
// Copy fields in to row
void atf_ci::targsrc_CopyIn(atf_ci::FTargsrc &row, dev::Targsrc &in) {
    row.targsrc = in.targsrc;
    row.comment = in.comment;
}

// --- atf_ci.FTargsrc.target.Get
algo::Smallstr16 atf_ci::target_Get(atf_ci::FTargsrc& targsrc) {
    algo::Smallstr16 ret(algo::Pathcomp(targsrc.targsrc, "/LL"));
    return ret;
}

// --- atf_ci.FTargsrc.src.Get
algo::Smallstr200 atf_ci::src_Get(atf_ci::FTargsrc& targsrc) {
    algo::Smallstr200 ret(algo::Pathcomp(targsrc.targsrc, "/LR"));
    return ret;
}

// --- atf_ci.FTargsrc.ext.Get
algo::Smallstr10 atf_ci::ext_Get(atf_ci::FTargsrc& targsrc) {
    algo::Smallstr10 ret(algo::Pathcomp(targsrc.targsrc, ".RR"));
    return ret;
}

// --- atf_ci.FTargsrc..Uninit
void atf_ci::FTargsrc_Uninit(atf_ci::FTargsrc& targsrc) {
    atf_ci::FTargsrc &row = targsrc; (void)row;
    atf_ci::FGitfile* p_src = atf_ci::ind_gitfile_Find(src_Get(row));
    if (p_src)  {
        c_targsrc_Remove(*p_src, row);// remove targsrc from index c_targsrc
    }
}

// --- atf_ci.FieldId.value.ToCstr
// Convert numeric value of field to one of predefined string constants.
// If string is found, return a static C string. Otherwise, return NULL.
const char* atf_ci::value_ToCstr(const atf_ci::FieldId& parent) {
    const char *ret = NULL;
    switch(value_GetEnum(parent)) {
        case atf_ci_FieldId_value          : ret = "value";  break;
    }
    return ret;
}

// --- atf_ci.FieldId.value.Print
// Convert value to a string. First, attempt conversion to a known string.
// If no string matches, print value as a numeric value.
void atf_ci::value_Print(const atf_ci::FieldId& parent, algo::cstring &lhs) {
    const char *strval = value_ToCstr(parent);
    if (strval) {
        lhs << strval;
    } else {
        lhs << parent.value;
    }
}

// --- atf_ci.FieldId.value.SetStrptrMaybe
// Convert string to field.
// If the string is invalid, do not modify field and return false.
// In case of success, return true
bool atf_ci::value_SetStrptrMaybe(atf_ci::FieldId& parent, algo::strptr rhs) {
    bool ret = false;
    switch (elems_N(rhs)) {
        case 5: {
            switch (u64(algo::ReadLE32(rhs.elems))|(u64(rhs[4])<<32)) {
                case LE_STR5('v','a','l','u','e'): {
                    value_SetEnum(parent,atf_ci_FieldId_value); ret = true; break;
                }
            }
            break;
        }
    }
    return ret;
}

// --- atf_ci.FieldId.value.SetStrptr
// Convert string to field.
// If the string is invalid, set numeric value to DFLT
void atf_ci::value_SetStrptr(atf_ci::FieldId& parent, algo::strptr rhs, atf_ci_FieldIdEnum dflt) {
    if (!value_SetStrptrMaybe(parent,rhs)) value_SetEnum(parent,dflt);
}

// --- atf_ci.FieldId.value.ReadStrptrMaybe
// Convert string to field. Return success value
bool atf_ci::value_ReadStrptrMaybe(atf_ci::FieldId& parent, algo::strptr rhs) {
    bool retval = false;
    retval = value_SetStrptrMaybe(parent,rhs); // try symbol conversion
    if (!retval) { // didn't work? try reading as underlying type
        retval = i32_ReadStrptrMaybe(parent.value,rhs);
    }
    return retval;
}

// --- atf_ci.FieldId..ReadStrptrMaybe
// Read fields of atf_ci::FieldId from an ascii string.
// The format of the string is the format of the atf_ci::FieldId's only field
bool atf_ci::FieldId_ReadStrptrMaybe(atf_ci::FieldId &parent, algo::strptr in_str) {
    bool retval = true;
    retval = retval && atf_ci::value_ReadStrptrMaybe(parent, in_str);
    return retval;
}

// --- atf_ci.FieldId..Print
// print string representation of atf_ci::FieldId to string LHS, no header -- cprint:atf_ci.FieldId.String
void atf_ci::FieldId_Print(atf_ci::FieldId & row, algo::cstring &str) {
    atf_ci::value_Print(row, str);
}

// --- atf_ci.TableId.value.ToCstr
// Convert numeric value of field to one of predefined string constants.
// If string is found, return a static C string. Otherwise, return NULL.
const char* atf_ci::value_ToCstr(const atf_ci::TableId& parent) {
    const char *ret = NULL;
    switch(value_GetEnum(parent)) {
        case atf_ci_TableId_dev_Builddir   : ret = "dev.Builddir";  break;
        case atf_ci_TableId_dev_Cfg        : ret = "dev.Cfg";  break;
        case atf_ci_TableId_dev_Gitfile    : ret = "dev.Gitfile";  break;
        case atf_ci_TableId_dev_Msgfile    : ret = "dev.Msgfile";  break;
        case atf_ci_TableId_dev_Noindent   : ret = "dev.Noindent";  break;
        case atf_ci_TableId_dmmeta_Ns      : ret = "dmmeta.Ns";  break;
        case atf_ci_TableId_dev_Readme     : ret = "dev.Readme";  break;
        case atf_ci_TableId_dev_Scriptfile : ret = "dev.Scriptfile";  break;
        case atf_ci_TableId_dmmeta_Ssimfile: ret = "dmmeta.Ssimfile";  break;
        case atf_ci_TableId_dev_Targsrc    : ret = "dev.Targsrc";  break;
    }
    return ret;
}

// --- atf_ci.TableId.value.Print
// Convert value to a string. First, attempt conversion to a known string.
// If no string matches, print value as a numeric value.
void atf_ci::value_Print(const atf_ci::TableId& parent, algo::cstring &lhs) {
    const char *strval = value_ToCstr(parent);
    if (strval) {
        lhs << strval;
    } else {
        lhs << parent.value;
    }
}

// --- atf_ci.TableId.value.SetStrptrMaybe
// Convert string to field.
// If the string is invalid, do not modify field and return false.
// In case of success, return true
bool atf_ci::value_SetStrptrMaybe(atf_ci::TableId& parent, algo::strptr rhs) {
    bool ret = false;
    switch (elems_N(rhs)) {
        case 7: {
            switch (u64(algo::ReadLE32(rhs.elems))|(u64(algo::ReadLE16(rhs.elems+4))<<32)|(u64(rhs[6])<<48)) {
                case LE_STR7('d','e','v','.','C','f','g'): {
                    value_SetEnum(parent,atf_ci_TableId_dev_Cfg); ret = true; break;
                }
                case LE_STR7('d','e','v','.','c','f','g'): {
                    value_SetEnum(parent,atf_ci_TableId_dev_cfg); ret = true; break;
                }
            }
            break;
        }
        case 9: {
            switch (algo::ReadLE64(rhs.elems)) {
                case LE_STR8('d','m','m','e','t','a','.','N'): {
                    if (memcmp(rhs.elems+8,"s",1)==0) { value_SetEnum(parent,atf_ci_TableId_dmmeta_Ns); ret = true; break; }
                    break;
                }
                case LE_STR8('d','m','m','e','t','a','.','n'): {
                    if (memcmp(rhs.elems+8,"s",1)==0) { value_SetEnum(parent,atf_ci_TableId_dmmeta_ns); ret = true; break; }
                    break;
                }
            }
            break;
        }
        case 10: {
            switch (algo::ReadLE64(rhs.elems)) {
                case LE_STR8('d','e','v','.','R','e','a','d'): {
                    if (memcmp(rhs.elems+8,"me",2)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Readme); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','r','e','a','d'): {
                    if (memcmp(rhs.elems+8,"me",2)==0) { value_SetEnum(parent,atf_ci_TableId_dev_readme); ret = true; break; }
                    break;
                }
            }
            break;
        }
        case 11: {
            switch (algo::ReadLE64(rhs.elems)) {
                case LE_STR8('d','e','v','.','G','i','t','f'): {
                    if (memcmp(rhs.elems+8,"ile",3)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Gitfile); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','M','s','g','f'): {
                    if (memcmp(rhs.elems+8,"ile",3)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Msgfile); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','T','a','r','g'): {
                    if (memcmp(rhs.elems+8,"src",3)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Targsrc); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','g','i','t','f'): {
                    if (memcmp(rhs.elems+8,"ile",3)==0) { value_SetEnum(parent,atf_ci_TableId_dev_gitfile); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','m','s','g','f'): {
                    if (memcmp(rhs.elems+8,"ile",3)==0) { value_SetEnum(parent,atf_ci_TableId_dev_msgfile); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','t','a','r','g'): {
                    if (memcmp(rhs.elems+8,"src",3)==0) { value_SetEnum(parent,atf_ci_TableId_dev_targsrc); ret = true; break; }
                    break;
                }
            }
            break;
        }
        case 12: {
            switch (algo::ReadLE64(rhs.elems)) {
                case LE_STR8('d','e','v','.','B','u','i','l'): {
                    if (memcmp(rhs.elems+8,"ddir",4)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Builddir); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','N','o','i','n'): {
                    if (memcmp(rhs.elems+8,"dent",4)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Noindent); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','b','u','i','l'): {
                    if (memcmp(rhs.elems+8,"ddir",4)==0) { value_SetEnum(parent,atf_ci_TableId_dev_builddir); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','n','o','i','n'): {
                    if (memcmp(rhs.elems+8,"dent",4)==0) { value_SetEnum(parent,atf_ci_TableId_dev_noindent); ret = true; break; }
                    break;
                }
            }
            break;
        }
        case 14: {
            switch (algo::ReadLE64(rhs.elems)) {
                case LE_STR8('d','e','v','.','S','c','r','i'): {
                    if (memcmp(rhs.elems+8,"ptfile",6)==0) { value_SetEnum(parent,atf_ci_TableId_dev_Scriptfile); ret = true; break; }
                    break;
                }
                case LE_STR8('d','e','v','.','s','c','r','i'): {
                    if (memcmp(rhs.elems+8,"ptfile",6)==0) { value_SetEnum(parent,atf_ci_TableId_dev_scriptfile); ret = true; break; }
                    break;
                }
            }
            break;
        }
        case 15: {
            switch (algo::ReadLE64(rhs.elems)) {
                case LE_STR8('d','m','m','e','t','a','.','S'): {
                    if (memcmp(rhs.elems+8,"simfile",7)==0) { value_SetEnum(parent,atf_ci_TableId_dmmeta_Ssimfile); ret = true; break; }
                    break;
                }
                case LE_STR8('d','m','m','e','t','a','.','s'): {
                    if (memcmp(rhs.elems+8,"simfile",7)==0) { value_SetEnum(parent,atf_ci_TableId_dmmeta_ssimfile); ret = true; break; }
                    break;
                }
            }
            break;
        }
    }
    return ret;
}

// --- atf_ci.TableId.value.SetStrptr
// Convert string to field.
// If the string is invalid, set numeric value to DFLT
void atf_ci::value_SetStrptr(atf_ci::TableId& parent, algo::strptr rhs, atf_ci_TableIdEnum dflt) {
    if (!value_SetStrptrMaybe(parent,rhs)) value_SetEnum(parent,dflt);
}

// --- atf_ci.TableId.value.ReadStrptrMaybe
// Convert string to field. Return success value
bool atf_ci::value_ReadStrptrMaybe(atf_ci::TableId& parent, algo::strptr rhs) {
    bool retval = false;
    retval = value_SetStrptrMaybe(parent,rhs); // try symbol conversion
    if (!retval) { // didn't work? try reading as underlying type
        retval = i32_ReadStrptrMaybe(parent.value,rhs);
    }
    return retval;
}

// --- atf_ci.TableId..ReadStrptrMaybe
// Read fields of atf_ci::TableId from an ascii string.
// The format of the string is the format of the atf_ci::TableId's only field
bool atf_ci::TableId_ReadStrptrMaybe(atf_ci::TableId &parent, algo::strptr in_str) {
    bool retval = true;
    retval = retval && atf_ci::value_ReadStrptrMaybe(parent, in_str);
    return retval;
}

// --- atf_ci.TableId..Print
// print string representation of atf_ci::TableId to string LHS, no header -- cprint:atf_ci.TableId.String
void atf_ci::TableId_Print(atf_ci::TableId & row, algo::cstring &str) {
    atf_ci::value_Print(row, str);
}

// --- atf_ci...SizeCheck
inline static void atf_ci::SizeCheck() {
}

// --- atf_ci...main
int main(int argc, char **argv) {
    try {
        lib_json::FDb_Init();
        algo_lib::FDb_Init();
        lib_git::FDb_Init();
        atf_ci::FDb_Init();
        algo_lib::_db.argc = argc;
        algo_lib::_db.argv = argv;
        algo_lib::IohookInit();
        atf_ci::MainArgs(algo_lib::_db.argc,algo_lib::_db.argv); // dmmeta.main:atf_ci
    } catch(algo_lib::ErrorX &x) {
        prerr("atf_ci.error  " << x); // there may be additional hints in DetachBadTags
        algo_lib::_db.exit_code = 1;
    }
    if (algo_lib::_db.last_signal) {
        algo_lib::_db.exit_code = 1;
    }
    try {
        atf_ci::FDb_Uninit();
        lib_git::FDb_Uninit();
        algo_lib::FDb_Uninit();
        lib_json::FDb_Uninit();
    } catch(algo_lib::ErrorX &) {
        // don't print anything, might crash
        algo_lib::_db.exit_code = 1;
    }
    // only the lower 1 byte makes it to the outside world
    (void)i32_UpdateMin(algo_lib::_db.exit_code,255);
    return algo_lib::_db.exit_code;
}

// --- atf_ci...WinMain
#if defined(WIN32)
int WINAPI WinMain(HINSTANCE,HINSTANCE,LPSTR,int) {
    return main(__argc,__argv);
}
#endif
