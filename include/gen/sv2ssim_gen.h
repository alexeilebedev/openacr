...
extern const char *sv2ssim_syntax;
} // gen:ns_field
// gen:ns_fwddecl2
...
sv2ssim::FField*     field_Find(i32 t) __attribute__((__warn_unused_result__, nothrow));
...
sv2ssim::FField&     field_qFind(i32 t) __attribute__((nothrow));
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
...
// Return true if index is empty
bool                 svtype_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
sv2ssim::FSvtype*    svtype_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// 'quick' Access row by row id. No bounds checking.
sv2ssim::FSvtype&    svtype_qFind(u64 t) __attribute__((nothrow));
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
...
// Return true if index is empty
bool                 zd_selfield_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
...
// Look up row by row id. Return NULL if out of range
sv2ssim::FBltin*     bltin_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// 'quick' Access row by row id. No bounds checking.
sv2ssim::FBltin&     bltin_qFind(u64 t) __attribute__((nothrow));
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
