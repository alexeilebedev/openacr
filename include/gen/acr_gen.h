...
struct FCheck { // acr.FCheck: Function to check for consistency
    ...
    ~FCheck();
private:
    // reftype of acr.FCheck.c_bad_rec prohibits copy
    // reftype of acr.FCheck.ary_name prohibits copy
...
// Return true if index is empty
bool                 zd_trec_EmptyQ(acr::FCtype& ctype) __attribute__((__warn_unused_result__, nothrow));
...
// Return true if index is empty
bool                 zd_selrec_EmptyQ(acr::FCtype& ctype) __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 zd_arg_EmptyQ(acr::FCtype& ctype) __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 zd_pline_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 zd_pdep_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Look up row by row id. Return NULL if out of range
acr::FCtype*         ctype_Find(i32 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FAnonfld*       anonfld_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FCdflt*         cdflt_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FField*         field_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Return true if index is empty
bool                 file_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
acr::FFile*          file_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Return true if index is empty
bool                 zd_all_selrec_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 zd_all_err_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 zd_sel_ctype_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Look up row by row id. Return NULL if out of range
acr::FSubstr*        substr_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FSsimfile*      ssimfile_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Return true if index is empty
bool                 tempkey_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
acr::FTempkey*       tempkey_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Return true if index is empty
bool                 zs_query_EmptyQ() __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 ssimsort_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
acr::FSsimsort*      ssimsort_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FSmallstr*      smallstr_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FFunique*       funique_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FBltin*         bltin_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Look up row by row id. Return NULL if out of range
acr::FCppfunc*       cppfunc_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Return true if index is empty
bool                 zd_frec_EmptyQ(acr::FFile& file) __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
// Return true if index is empty
bool                 zd_child_EmptyQ(acr::FPline& pline) __attribute__((__warn_unused_result__, nothrow));
// If index empty, return NULL. Otherwise return pointer to first element in index
...
struct FPrint { // acr.FPrint: Print function
    ...
    ~FPrint();
private:
    // reftype of acr.FPrint.ind_printattr prohibits copy
    // x-reference on acr.FPrint.ind_printattr prevents copy
    // cascdel on acr.FPrint.ind_printattr prevents copy
...
struct FQuery { // acr.FQuery
    ...
    algo::cstring   field;           //   ""  Field name (SQL regex as string)
    algo::cstring   value;           //   ""  Target value (SQL regex as string)
    acr::Queryop    queryop;         //
    algo::cstring   new_val;         //   ""
    bool            pk;              //   false  true if value is primary key, not column
    ...
    algo::cstring   regx_ssimfile;   //   ""  Full ssimfile name (SQL regex as string)
    algo::cstring   ctype;           //   ""  Full ctype name (SQL regex as string)
...
struct FRun { // acr.FRun: Function to read/process query
    ...
    u32              c_child_n;       // array of pointers
    u32              c_child_max;     // capacity of allocated array
    algo_lib::Regx   field_regx;      //
    algo_lib::Regx   value_regx;      //
    FRun();
    ~FRun();
private:
    // reftype of acr.FRun.c_ctype prohibits copy
    // reftype of acr.FRun.c_field prohibits copy
    // reftype of acr.FRun.c_rec prohibits copy
    // ... and several other reasons
...
struct FWrite { // acr.FWrite: Print function
    ...
    ~FWrite();
private:
    // reftype of acr.FWrite.c_cmtrec prohibits copy
