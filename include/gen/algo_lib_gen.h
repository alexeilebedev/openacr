...
enum algo_lib_FieldIdEnum {               // algo_lib.FieldId.value
    ...
    ,algo_lib_FieldId_sig           = 4
    ,algo_lib_FieldId_signature     = 5
    ,algo_lib_FieldId_expr          = 6
    ,algo_lib_FieldId_state         = 7
    ,algo_lib_FieldId_front         = 8
    ,algo_lib_FieldId_next_front    = 9
    ,algo_lib_FieldId_accept        = 10
    ,algo_lib_FieldId_start         = 11
    ,algo_lib_FieldId_parseerror    = 12
    ,algo_lib_FieldId_accepts_all   = 13
    ,algo_lib_FieldId_value         = 14
};

enum { algo_lib_FieldIdEnum_N = 15 };
...
struct Bitset { // algo_lib.Bitset
    ...
    ~Bitset();
private:
    // reftype of algo_lib.Bitset.ary prohibits copy
...
struct Cmdline { // algo_lib.Cmdline: *can't move this to command namespace because of circular dependency*
    ...
    bool   sig;         //   false  Show signatures and exit
    bool   signature;   //   false  Alias for -sig
    Cmdline();
};
...
struct CsvParse { // algo_lib.CsvParse
    ...
    ~CsvParse();
private:
    // reftype of algo_lib.CsvParse.ary_tok prohibits copy
...
private:
    ...
    // user-defined fcleanup on algo_lib.FLockfile.fildes prevents copy
...
// Return true if index is empty
bool                 imtable_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
algo_lib::FImtable*  imtable_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// Return true if index is empty
bool                 dispsigcheck_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
algo_lib::FDispsigcheck* dispsigcheck_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
private:
    ...
    // reftype of algo_lib.FIohook.callback prohibits copy
...
struct FTxttbl { // algo_lib.FTxttbl: Table row. Todo: absolute index for cells?
    ...
    ~FTxttbl();
private:
    // reftype of algo_lib.FTxttbl.c_txtrow prohibits copy
    // x-reference on algo_lib.FTxttbl.c_txtrow prevents copy
    // cascdel on algo_lib.FTxttbl.c_txtrow prevents copy
...
private:
    ...
    // user-defined fcleanup on algo_lib.InTextFile.file prevents copy
...
private:
    ...
    // reftype of algo_lib.Regx.state prohibits copy
...
private:
    ...
    // reftype of algo_lib.RegxParse.ary_expr prohibits copy
...
struct RegxState { // algo_lib.RegxState
    ...
    ~RegxState();
private:
    // reftype of algo_lib.RegxState.ch_class prohibits copy
...
struct Replscope { // algo_lib.Replscope
    ...
    ~Replscope();
private:
    // reftype of algo_lib.Replscope.ind_replvar prohibits copy
    // x-reference on algo_lib.Replscope.ind_replvar prevents copy
    // cascdel on algo_lib.Replscope.ind_replvar prevents copy
    // ... and several other reasons
...
struct Tabulate { // algo_lib.Tabulate: Function to tabulate a string
    ...
    ~Tabulate();
private:
    // reftype of algo_lib.Tabulate.width prohibits copy
