...
extern const char *atf_fuzz_syntax;
} // gen:ns_field
// gen:ns_fwddecl2
...
bool                 fuzzstrat_EmptyQ() __attribute__((nothrow));
// Look up row by row id. Return NULL if out of range
atf_fuzz::FFuzzstrat* fuzzstrat_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
...
atf_fuzz::FFuzzstrat& fuzzstrat_qFind(u64 t) __attribute__((nothrow));
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
...
// Look up row by row id. Return NULL if out of range
atf_fuzz::FTarget*   target_Find(u64 t) __attribute__((__warn_unused_result__, nothrow));
// Return pointer to last element of array, or NULL if array is empty
...
// 'quick' Access row by row id. No bounds checking.
atf_fuzz::FTarget&   target_qFind(u64 t) __attribute__((nothrow));
// Insert row into all appropriate indices. If error occurs, store error
// in algo_lib::_db.errtext and return false. Caller must Delete or Unref such row.
...
private:
    ...
    FFuzzstrat();
    // reftype of atf_fuzz.FFuzzstrat.step prohibits copy
