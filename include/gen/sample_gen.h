...
enum sample_FieldIdEnum {        // sample.FieldId.value
     sample_FieldId_key1    = 0
    ,sample_FieldId_key2    = 1
    ,sample_FieldId_key3    = 2
    ,sample_FieldId_value   = 3
};

enum { sample_FieldIdEnum_N = 4 };
...
namespace sample { struct Reckey; }
...
struct FDb { // sample.FDb
    ...
    algo::SchedTime   bh_rec_next;             // sample.FDb.bh_rec                     Next invocation time
    algo::SchedTime   bh_rec_delay;            // sample.FDb.bh_rec                     Delay between invocations
    sample::trace     trace;                   //
};
...
sample::FRec*        ind_rec_Find(const sample::Reckey& key) __attribute__((__warn_unused_result__, nothrow));
// Look up row by key and return reference. Throw exception if not found
// func:sample.FDb.ind_rec.FindX
sample::FRec&        ind_rec_FindX(const sample::Reckey& key);
...
// func:sample.FDb.bh_rec.FirstChanged
void                 bh_rec_FirstChanged() __attribute__((nothrow));
// func:sample.FDb.bh_rec.Step
// this function is 'extrn' and implemented by user
void                 bh_rec_Step() __attribute__((nothrow));
...
// func:sample.FDb.bh_rec.SetDelay
void                 bh_rec_SetDelay(algo::SchedTime delay) __attribute__((nothrow));
...
void                 Reckey_Print(sample::Reckey& row, algo::cstring& str) __attribute__((nothrow));
...
inline algo::cstring &operator <<(algo::cstring &str, const sample::Reckey &row);// cfmt:sample.Reckey.String
